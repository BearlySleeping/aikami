# gdlint: disable = max-public-methods , load-constant-name , class-definitions-order , unused-argument , no-else-return , no-elif-return
@tool
...
var outline_initially_closed: bool = false
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if open_outline_popup.matches_event(event):
		...
		old_text = filter_txt.text
		filter_txt.text = ""

		outline_popup = POPUP_SCRIPT.new()
		outline_popup.input_listener = _input

		outline_initially_closed = !outline_container.visible
		...
		var script_editor: ScriptEditor = get_editor_interface().get_script_editor()
		outline_popup.popup_hide.connect(_on_outline_popup_popup_hide)
...
func _on_outline_popup_popup_hide() -> void:
	# gdformat: disable
	if outline_initially_closed:
...
func add_to_outline_ext(
	items: Array[String], icon_callable: Callable, type: String, modifier: String = ""
):
