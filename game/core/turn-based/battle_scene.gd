...
func _on_targeting_started(p_valid_targets: Array[Unit]) -> void:
	...
	while TurnManager.current_state == TurnManager.BattleState.SELECTING_TARGET:
		if Input.is_action_just_pressed("ui_up") and current_target_index > 0:
			...
			valid_targets[current_target_index].focus()
		elif Input.is_action_just_pressed("ui_down") and current_target_index < valid_targets.size() - 1:
			valid_targets[current_target_index].unfocus()
			current_target_index += 1
			valid_targets[current_target_index].focus()
		elif Input.is_action_just_pressed("ui_accept"):
			TurnManager.execute_command(TurnManager.selected_command, valid_targets[current_target_index])
			valid_targets[current_target_index].unfocus()
			break
