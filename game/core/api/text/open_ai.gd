...
func call_text_function(
	...
	var use_stream := request.use_stream
	var options := _get_options(request.messages, use_stream)

	var tool := _to_gpt_function_request(request)
	(
		options
		. merge(
			{
	...
	var gpt_response := await _make_gpt_request(options, true, use_stream)
	Logger.info("call_text_function:_stream_text", _stream_text)
	var error: Variant = gpt_response[1]
	if error:
		printerr("call_npc_dialogue_basic:error\n" + str(error))
		response.error = error
		return response
	var parsed_response: Dictionary = gpt_response[0]
	var response_message: Variant = parsed_response.choices[0].message
	# [{ id:String, type:"function", function: {name: String, arguments: Dictionary } }]
	var tool_calls: Array = response_message.tool_calls
	var first_function: Dictionary = tool_calls[0]
	var arguments: String = first_function.function.arguments
	response.data = JSON.parse_string(arguments)
	Logger.info("call_text_function:response\n", response.data)
	return response
