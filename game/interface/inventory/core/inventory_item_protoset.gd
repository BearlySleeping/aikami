...
## Parses the given json string and generates a new prototypes dictionary.
func parse(json: String) -> void:
	_prototypes.clear()

	var test_json_conv: JSON = JSON.new()
	assert(test_json_conv.parse(json) == OK, "Failed to parse JSON!")
	var parse_result: Array = test_json_conv.data

	for prototype: Dictionary in parse_result:
		...
		var id: String = prototype[KEY_ID]
