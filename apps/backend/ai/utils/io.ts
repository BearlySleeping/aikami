 ...
 */
export const openFile = async (options: { filePath: string }): Promise<void> => {
...
const getOutputDirectory = (options: { type: AIType; modelName?: string }): string => {
  const { type } = options;
...
const getUniqueFileName = (options: { type: AIType; fileExtension?: string }): string => {
  const extension = options.fileExtension || getDefaultFileExtension(options.type);
