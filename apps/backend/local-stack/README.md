...
**Point the client at it.** The client (`client` profile — the game
...
### Contributor path (source builds)

Want to build the engines from source or edit the stack itself? Clone the
repo — this is the **contributor** path, not the primary one:
...
bun run stack init   # interactive hardware wizard
# or non-interactive:
# bun run stack init --yes --backend cuda --modalities text,voice
docker compose up -d
```

`model-fetcher`, `voice`, and `client` default to pulling the published
`ghcr.io/bearlysleeping/aikami-*` images; `docker compose build <service>`
builds from source instead. This repo-local path also exposes the full
`stack init` flag set documented below.
