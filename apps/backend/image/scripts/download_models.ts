...
const resolveUrl = (): string =>
	Bun.argv[2] ?? process.env.AIKAMI_DEFAULT_CHECKPOINT_URL ?? '';
...
//
//   Miaomiao Pixel vPred11:
//     https://civitai.com/models/1180112/miaomiao-pixel
//
//   RDXL Pixel Art (Pony v2):
//     https://civitai.com/models/638637/rdxl-pixel-art
...
const downloadModel = async (entry: ModelEntry): Promise<void> => {
	...
	try {
		while (true) {
			...
			const speed =
				elapsed > 0 ? formatBytes(downloaded / elapsed) : '...';
			const pct =
				totalSize > 0
					? ((downloaded / totalSize) * 100).toFixed(1)
					: '?';
			process.stdout.write(
				`\r  ${formatBytes(downloaded)} / ${totalLabel} (${pct}%) @ ${speed}/s`,
			);
		}
...
const main = async (): Promise<void> => {
	...
	if (entries.length === 0) {
		...
		console.log(
			'  Or set AIKAMI_DEFAULT_CHECKPOINT_URL env var',
		);
		process.exit(0);
	}
