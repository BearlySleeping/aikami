...
export const reconcileBucket = async (options: SyncOptions): Promise<{ checked: number }> => {
	...
	console.log(`🪣 Reconciling ${entry.bucketName} (${mode})...`);
	...
	try {
		...
		console.error(`storage:sync failed: ${stderr || (error as Error).message}`);
		throw error;
	}
};
...
const main = async (): Promise<void> => {
	...
	if (mode === 'production') {
		...
		if (!process.stdin.isTTY) {
			if (!isYes) {
				console.error('storage:sync: refusing to run against production non-interactively without --yes.');
				process.exit(1);
			}
		} else if (!(await confirmProduction())) {
			console.error('storage:sync: aborted.');
			process.exit(1);
		}
	}
	...
	try {
		...
		console.error(`storage:sync failed: ${(error as Error).message}`);
		process.exit(1);
	}
};
