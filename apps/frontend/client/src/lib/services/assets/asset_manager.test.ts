...
const createMockRegistry = () => {
  ...
  return {
    ...
    // ── C-510 generated-asset write seam ──────────────────────────────
    isSeedTag: async (tag: string): Promise<boolean> => {
      const record = records.get(tag);
      return record !== undefined && record.packId !== 'generated';
    },
    registerGenerated: async (asset: {
      tag: string;
      hash: string;
      sizeBytes: number;
      category: string;
      ...
      const existing = records.get(asset.tag);
      if (existing && existing.packId !== 'generated') {
        throw new Error(
          `Cannot register the generated asset "${asset.tag}": a catalog (seed) asset already owns that tag (hash ${existing.hash}). Regenerate under a distinct tag.`,
        );
      }
      const unchanged = existing !== undefined && existing.hash === asset.hash;
      let version = 1;
      if (existing) {
        version = unchanged ? existing.version : existing.version + 1;
      }
      if (!unchanged) {
        records.set(asset.tag, {
          id: asset.tag,
          packId: 'generated',
          ...
          license: 'unknown',
          attribution: asset.provenanceSource,
          tags: [],
        });
      }
      const assetSources = sources.get(asset.tag) ?? [];
      sources.set(asset.tag, [
        ...assetSources.filter((source) => source.backend !== 'local-generated'),
        {
          assetId: asset.tag,
          backend: 'local-generated',
          url: `local-generated:${asset.hash}`,
          priority: -1,
        },
      ]);
...
// ---------------------------------------------------------------------------
// C-510 AC-4: generated bytes register and resolve
// ---------------------------------------------------------------------------
...
const generatedFixture = async (prompt: string, bytes = new Uint8Array([1, 2, 3, 4])) => {
  ...
  const asset: GeneratedAsset = await toGeneratedAsset(result, requireRecipe('prop'), 'sdcpp');
  return { asset, bytes };
};

describe('AssetManager.registerGenerated (C-510 AC-4)', () => {
  beforeEach(async () => {
    await assetManager.teardown();
  });
  ...
  test('writes verified bytes, a registry row and a priority -1 source, then resolves', async () => {
    ...
    const { asset, bytes } = await generatedFixture('rusty iron gate');
    const result = await assetManager.registerGenerated(asset, bytes);

    expect(result.registered).toBe(true);
    expect(result.version).toBe(1);

    // Bytes are in the cache under the content hash.
    expect(await backend.has(asset.sha256)).toBe(true);

    // Registry row + local-generated source at priority -1.
    ...
    expect(sources).toEqual([
      {
        assetId: asset.tag,
        backend: 'local-generated',
        url: `local-generated:${asset.sha256}`,
        priority: -1,
      },
    ]);

    // The synchronous seam the expression resolver uses returns a usable URL.
    expect(assetManager.peekBlobUrl(asset.tag)).toBeTruthy();
    expect(assetManager.acquireUrl(asset.tag)).toBeTruthy();
    expect(assetManager.acquireUrl(asset.tag)?.startsWith('blob:')).toBe(true);
  });

  test('the source URL is a stable cache key, never a blob: URL', async () => {
    const registry = createMockRegistry();
    const backend = createMockBackend();
    await assetManager.initialize({
      registry: registry as never,
    ...
    await assetManager.registerGenerated(asset, bytes);

    const sources = await registry.listSources(asset.tag);
    expect(sources[0]?.url).not.toContain('blob:');
    expect(sources[0]?.url).toBe(`local-generated:${asset.sha256}`);
  });
  ...
  test('re-registering identical bytes is idempotent', async () => {
    ...
    const { asset, bytes } = await generatedFixture('a gate');
    const first = await assetManager.registerGenerated(asset, bytes);
    const second = await assetManager.registerGenerated(asset, bytes);

    expect(first.version).toBe(1);
    expect(second.version).toBe(1);
    expect(second.unchanged).toBe(true);
    expect((await registry.listSources(asset.tag)).length).toBe(1);
  });

  test('registering different bytes for the same tag bumps the version', async () => {
    const registry = createMockRegistry();
    const backend = createMockBackend();
    await assetManager.initialize({
      registry: registry as never,
  ...
  test('a hash mismatch is rejected before anything is cached or registered', async () => {
    ...
    const { asset } = await generatedFixture('a gate');
    const tampered = { ...asset, sha256: 'f'.repeat(64) };

    await expect(
      assetManager.registerGenerated(tampered, new Uint8Array([1, 2, 3, 4])),
    ).rejects.toThrow(/hash mismatch/i);

    expect(await registry.findById(asset.tag)).toBeUndefined();
    expect(backend._files.size).toBe(0);
  });

  test('a tag owned by the boot seed is rejected, not silently re-pointed', async () => {
    const registry = createMockRegistry();
    const backend = createMockBackend();
    await assetManager.initialize({
      registry: registry as never,
    ...
    // Simulate a catalog asset that already owns the tag.
    registry._records.set('props:rusty-iron-gate', {
      id: 'props:rusty-iron-gate',
      packId: 'props',
      category: 'props',
      hash: 'a'.repeat(64),
      version: 3,
    ...
    expect(registry._records.get('props:rusty-iron-gate')?.hash).toBe('a'.repeat(64));
    expect(backend._files.size).toBe(0);
  });

  test('an oversize result fails fast', async () => {
    const registry = createMockRegistry();
    const backend = createMockBackend();
    await assetManager.initialize({
      registry: registry as never,
    ...
    const result: GenerationResult = {
      ...
      mimeType: 'image/png',
      engine: 'sdcpp',
      metadata: { bytes: big.length, prompt: 'a gate' },
    };
    const asset = await toGeneratedAsset(result, requireRecipe('prop'), 'sdcpp');

    await expect(assetManager.registerGenerated(asset, big)).rejects.toThrow(/over the 50 MB cap/);
  });

  test('an uninitialised manager is a no-op rather than a crash', async () => {
    const { asset, bytes } = await generatedFixture('a gate');
    const result = await assetManager.registerGenerated(asset, bytes);
  ...
  test('registration resolves offline — no fetch is issued', async () => {
    const registry = createMockRegistry();
    const backend = createMockBackend();
    await assetManager.initialize({
      registry: registry as never,
    ...
    try {
      const { asset, bytes } = await generatedFixture('a gate');
      await assetManager.registerGenerated(asset, bytes);
      const url = await assetManager.resolve(asset.tag);
