...
class PersonaService extends BaseFrontendClass implements PersonaServiceInterface {
  async hasPersona(): Promise<boolean> {
    ...
    const personas = await personaFirestoreRepository.getDocumentsByCollection({
      uid: user.id,
    });
  ...
  async getPersonas(uid: string): Promise<PersonaData[]> {
    return await personaFirestoreRepository.getDocumentsByCollection({ uid });
  }
  ...
  async getActivePersona(): Promise<PersonaData | undefined> {
    ...
    const personas = await personaFirestoreRepository.getDocumentsByCollection({ uid: user.id });
