...
import type { NpcAwarenessServiceInterface } from './npc_awareness_service.svelte.ts';
...
mock.module('./npc_service.svelte.ts', () => ({
  npcService: {
    get: mock((options: { npcId: string }) => {
      ...
      return Promise.resolve(npcs.get(options.npcId));
    }),
  },
}));
...
let npcAwarenessService: NpcAwarenessServiceInterface;
...
describe('NpcAwarenessService', () => {
  beforeEach(async () => {
    ...
    currentLocation.npcIds = ['npc_merchant', 'npc_guard', 'npc_party_member'];
    ({ npcAwarenessService } = await import('./npc_awareness_service.svelte.ts'));
  });
  ...
  it('should expose nearbyNpcIds from current location', () => {
