// apps/frontend/client/src/lib/services/database/npc.svelte.ts
import { chatFirestoreRepository, npcFirestoreRepository } from '@aikami/frontend/firestore';
import {
  BaseFrontendClass,
...
class NpcService extends BaseFrontendClass<NpcServiceOptions> implements NpcServiceInterface {
  async getSystemNpcs(): Promise<NpcData[]> {
    return await npcFirestoreRepository.getDocumentsByCollection(undefined);
  }
  ...
  async getPublicNpcs(): Promise<NpcData[]> {
    const userNpcs = await npcFirestoreRepository.getDocumentsByCollection(undefined);
  ...
  async get(options: { npcId: string }): Promise<NpcData | undefined> {
    ...
    const userNpc = await npcFirestoreRepository.getDocument({ npcId });
    if (userNpc) {
      return userNpc;
    }

    return await npcFirestoreRepository.getDocument({ npcId });
  }
  ...
  async createNpc(options: { data: Partial<NpcCreateData>; uid: string }): Promise<string> {
    ...
    const npcId = await npcFirestoreRepository.addDocument({
  ...
  async updateNpc(options: { npcId: string; data: Partial<NpcData> }): Promise<void> {
    ...
    await npcFirestoreRepository.updateDocument({
  ...
  async deleteNpc(options: { npcId: string; deleteChatHistory?: boolean }): Promise<void> {
    ...
    await npcFirestoreRepository.deleteDocument({ npcId });

    if (deleteChatHistory) {
      const uid = authService.uid;
      if (uid) {
        // Query all chats for this NPC+user pair, then delete each one
        const chats = await chatFirestoreRepository.getDocumentsByQuery({
          getCollectionPathArgument: undefined,
          filters: [
        ...
        for (const chat of chats) {
          await chatFirestoreRepository.deleteDocument({ chatId: chat.id });
        }
      }
    }
  }
  ...
  async forkNpc(options: { systemNpcId: string; uid: string }): Promise<string> {
    ...
    const systemNpc = await npcFirestoreRepository.getDocument({ npcId: systemNpcId });
    if (!systemNpc) {
