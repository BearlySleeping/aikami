...
export const BATCHABLE_AGENT_IDS: ReadonlySet<string> = new Set([
  'world-state',
  'quest-tracker',
  'expression',
  'cyoa',
...
export type BatchedAgentOutput = {
  success: boolean;
  output?: unknown;
  error?: string;
};
...
/**
 * Runs one combined analysis call for the given batchable agents and returns
 * one `AgentRunResult` per agent. On failure every agent reports the same
 * error, so the pipeline can fall back to individual runs.
 */
