...
export type AuthServiceInterface = BaseFrontendClassInterface & {
   ...
   * @param callbackURL Optional URL to redirect to after sign-in (defaults to /).
   * @returns A promise that resolves with the social sign-in response.
   */
  socialSignIn(provider: SignInProviderName, callbackURL?: string): Promise<SocialSignInResponse>;
    ...
    if (!isTauri()) {
      try {
        await socialSignInRedirect(
          provider as import('@aikami/types').SignInSocialProvider,
          callbackURL,
        );
      } catch (error) {
