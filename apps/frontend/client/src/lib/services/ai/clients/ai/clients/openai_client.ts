// packages/frontend/engine/src/ai_clients/ai/clients/openai_client.ts
...
/**
 * OpenAI cloud provider — routes all AI requests through the backend proxy.
 *
 * API keys NEVER leave the server. All requests go through a
 * {@link GameApiClientInterface} instance that calls the backend
 * `prompt_ai` / `generate_image` Firebase Functions.
 *
 * Does NOT bundle the `openai` npm SDK. Zero direct OpenAI dependencies.
 */
class OpenAiClient implements FrontendAiInterface {
   ...
   */
  constructor(apiClient: GameApiClientInterface, model: string = 'gpt-4o') {
  ...
  async synthesizeSpeech(_text: string, _options?: TtsOptions): Promise<SpeechResult> {
    throw new Error('OpenAiClient does not support speech synthesis. Use LocalTtsClient instead.');
  }
