// apps/frontend/client/src/lib/services/ai/ai_config.ts
 ...
 */
async function createConfiguredAiClient(
  apiClient?: GameApiClientInterface,
): Promise<FrontendAiInterface> {
  const { createAiClient } = await import('./clients/index.ts');
