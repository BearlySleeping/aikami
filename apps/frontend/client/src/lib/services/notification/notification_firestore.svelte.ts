import {
  type NotificationFirestoreRepositoryInterface,
  notificationFirestoreRepository,
} from '@aikami/frontend/firestore/notification.ts';
...
export type NotificationServiceOptions = BaseFrontendClassOptions & {
  firestore: NotificationFirestoreRepositoryInterface;
};
  ...
  implements NotificationServiceInterface
{
  ...
  private get _firestore(): NotificationFirestoreRepositoryInterface {
    return this._options.firestore;
  }
  ...
  async clearNotifications(options: { uid: string }): Promise<boolean> {
    ...
    try {
      ...
      await this._firestore.deleteDocuments(
  ...
  async listenForNotifications(uid?: string): Promise<void> {
    ...
    const observable = await this._firestore.getDocumentsStreamByQuery({
...
export const notificationService: NotificationServiceInterface = NotificationService.create({
  firestore: notificationFirestoreRepository,
