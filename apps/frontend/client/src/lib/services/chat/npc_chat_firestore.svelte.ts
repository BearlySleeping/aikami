// apps/frontend/client/src/lib/services/database/npc-chat.svelte.ts
  ...
  implements NpcChatServiceInterface
{
  async getChat(options: { uid: string; npcId: string }): Promise<ChatData | undefined> {
    ...
    try {
      const results = await chatFirestoreRepository.getDocumentsByQuery({
  ...
  async getOrCreateChat(options: {
    ...
    try {
      const chatId = await chatFirestoreRepository.addDocument({
        getCollectionPathArgument: undefined,
        createData: {
          npcId,
          npcName,
  ...
  async addMessage(options: {
    ...
    try {
      const existingChat = await chatFirestoreRepository.getDocument({ chatId });
      ...
      if (existingChat) {
        ...
        await chatFirestoreRepository.updateDocument({
        ...
        await chatFirestoreRepository.addDocument({
          getCollectionPathArgument: undefined,
          createData: {
            npcId,
            npcName: npcId,
  ...
  async deleteChatById(options: { chatId: string }): Promise<void> {
    ...
    try {
      await chatFirestoreRepository.deleteDocument({ chatId });
    } catch (error) {
  ...
  > {
    const { chatId } = options;
    try {
      const chat = await chatFirestoreRepository.getDocument({ chatId });
      return chat;
    } catch (error) {
