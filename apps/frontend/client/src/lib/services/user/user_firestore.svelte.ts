import {
  type UserFirestoreRepositoryInterface,
  userFirestoreRepository,
} from '@aikami/frontend/firestore/user.ts';
import {
  BaseFrontendClass,
...
export type UserServiceOptions = BaseFrontendClassOptions & {
  firestore: UserFirestoreRepositoryInterface;
};
...
{
  private get _firestore(): UserFirestoreRepositoryInterface {
    return this._options.firestore;
  }

  async getUser(uid: string): Promise<undefined | UserData> {
    try {
      return await this._firestore.getDocument({ uid });
    } catch (error) {
  ...
  async getUserObservable(uid: string): Promise<DocumentObservable<UserData>> {
    ...
    return await this._firestore.getDocumentStream({ uid });
  }
...
export const userService: UserServiceInterface = UserService.create({
  firestore: userFirestoreRepository,
