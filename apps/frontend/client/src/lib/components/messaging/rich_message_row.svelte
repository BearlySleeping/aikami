    ...
    <div class="group flex gap-2 {alignRight ? 'flex-row-reverse' : 'flex-row'}">
      <div class="relative flex max-w-[75%] flex-col gap-0.5">
        ...
        {#if !readOnly}
          ...
          <div
            class="absolute top-full z-10 flex gap-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100 focus-within:opacity-100 max-sm:static max-sm:mt-0.5 max-sm:opacity-100 {alignRight ? 'right-0 justify-end' : 'left-0 justify-start'}"
          >
