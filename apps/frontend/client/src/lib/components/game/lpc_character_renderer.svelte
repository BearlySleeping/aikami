<script lang="ts">
  ...
  // -----------------------------------------------------------------------
  // TextureManager — instantiated lazily on first use (dynamic import from
  // engine to avoid static bundling for the dev sandbox)
  // -----------------------------------------------------------------------

  let _textureManager: import('@aikami/frontend/engine').TextureManager | undefined;

  /** Lazily initializes the TextureManager from the engine module. */
  const _getTextureManager = async () => {
    if (!_textureManager) {
