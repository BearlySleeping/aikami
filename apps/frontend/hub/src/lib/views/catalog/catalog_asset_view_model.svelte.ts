    ...
    try {
      ...
      switch (kind) {
        case 'lpc': {
          ...
          this.previewComponent = LpcPreview as unknown as ComponentType; // guard-ignore lint/type-safety/casting: dynamic component type resolution from catalog manifest
          this.previewProps = {
            resolver,
            allSlots,
            initialState,
        ...
        case 'tileset': {
          ...
          this.previewComponent = TilesetPreview as unknown as ComponentType; // guard-ignore lint/type-safety/casting: dynamic component type resolution from catalog manifest
          this.previewProps = {
            resolver,
            tag,
            width: 320,
        ...
        case 'map': {
          ...
          this.previewComponent = MapPreview as unknown as ComponentType; // guard-ignore lint/type-safety/casting: dynamic component type resolution from catalog manifest
          this.previewProps = { resolver, mapTag: tag, width: 320, height: 320, zoom: 1 };
          break;
        }
        case 'prop': {
          const { PropPreview } = await import('@aikami/frontend-preview');
          this.previewComponent = PropPreview as unknown as ComponentType; // guard-ignore lint/type-safety/casting: dynamic component type resolution from catalog manifest
