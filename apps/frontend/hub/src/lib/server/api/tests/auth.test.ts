...
//
// C-426 AC-4: the hub's login flow is served by Better Auth mounted on the
// Elysia app at /api/auth/*, backed by D1.
//
// The Elysia app is a module singleton; the D1 database is injected per-test
// via setBetterAuthEnv with a mock D1Database backed by an in-memory libsql
...
// (better_auth.ts, index.ts) is loaded — hence dynamic imports below.
...
describe('hub Better Auth mount (AC-4)', () => {
  ...
  test('sign-in via /api/auth/sign-in/email sets a session cookie and get-session recognizes it', async () => {
    ...
    const getRes = await app.handle(get('/api/auth/get-session', cookie));
  ...
  test('a signed-out get-session returns no user', async () => {
    const res = await app.handle(get('/api/auth/get-session', 'better-auth.session_token=invalid'));
    expect(res.status).toBe(200);
