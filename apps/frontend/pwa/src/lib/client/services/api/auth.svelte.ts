...
export class AuthService extends BaseFrontendClass<AuthServiceOptions>
  ...
  async socialSignIn(
    ...
    try {
      ...
      const response = await this._auth.signInWithPopup(
        toAuthProviderId(provider),
      )
  ...
  async registerUser(registerForm: RegisterForm): Promise<boolean> {
    try {
      ...
      const user = await this._auth.signInWithCustomToken(
