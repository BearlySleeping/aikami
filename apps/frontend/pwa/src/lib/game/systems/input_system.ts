 ...
 * the Svelte UI) and for keyboard events on the canvas element. This system
 * does NOT run per-frame — it reacts to discrete input events.
 *
...
const setupInput = (world: World, playerEntityId: number, bridge: EngineBridge): (() => void) => {
  ...
  // Listen for MOVE_PLAYER and STOP_PLAYER commands from the Svelte UI.
  // Cast through `never` because the EngineBridge.on() accepts GameEvent
  // types, but here we're using the internal onCommand() path. In
  // production, the game engine registers command handlers via onCommand.
  // For now, this system registers directly with the bridge's internal
  // routing by treating commands as a special bridge channel.
