// apps/frontend/pwa/src/lib/game/game-world.ts
import { createWorld } from 'bitecs';
import type { Application } from 'pixi.js';
import { registerNPCDialogObservers } from './components/npc-dialog.ts';
...
import { createPlayer } from './entities/create-player.ts';
import { createTestSprite } from './entities/create-test-sprite.ts';
import type { PixiAppOptions } from './pixi-app.ts';
import { createPixiApp } from './pixi-app.ts';
import { updateDialogTriggers } from './systems/dialog-trigger-system.ts';
import { setupInput } from './systems/input-system.ts';
import { updateMovement } from './systems/movement-system.ts';
import { updateRender } from './systems/render-system.ts';
 ...
 *
 * Zero Svelte imports. Zero `$state` runes. Pure imperative TypeScript.
 */
class GameWorld {
   ...
   * is exclusively owned by PixiJS — Svelte must NOT touch it after this.
   *
   ...
   *
   * Call this when the Svelte component is unmounted to prevent memory
