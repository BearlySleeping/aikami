...
import { NPCDialog, registerNPCDialogObservers } from '../components/npc-dialog.ts';
...
import { MockEngineBridge } from '../engine-bridge.ts';
import { createNPC } from '../entities/create-npc.ts';
...
describe('GameWorld — dialog trigger system', () => {
  ...
  it('emits NPC_DIALOG_START when player enters interaction radius', () => {
    ...
    // Verify playerInRange was set to true in the SoA array
  ...
  it('emits NPC_DIALOG_END when player leaves interaction radius', () => {
    ...
    addComponent(
      ...
      set(NPCDialog, {
        ...
        dialog: 'Bye!',
        interactionRadius: 5,
        playerInRange: true, // was previously in range
      }),
    );
  ...
  it('safely handles player without Position component', () => {
    const playerEid = addEntity(world); // no Position
