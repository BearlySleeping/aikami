// apps/frontend/pwa/src/lib/data/lpc_url_config.ts
...
import { LpcAnimationState, LpcDirection } from '@aikami/frontend/engine';
...
export const searchParamsToLpcState = (params: URLSearchParams): LpcUrlState => {
  ...
  if (rawState !== null) {
    ...
    if (!Number.isNaN(parsed) && parsed in LpcAnimationState) {
  ...
  if (rawDir !== null) {
    ...
    if (!Number.isNaN(parsed) && parsed in LpcDirection) {
