...
>
  <div class="card w-full max-w-md bg-base-100 shadow-xl">
    <div class="card-body">
      <h1 class="text-3xl font-bold text-center mb-6">{t.register()}</h1>
      ...
      <div class="flex flex-col gap-2 mb-6">
        <button
          ...
          onclick={() => viewModel.socialRegister("google")}
        >
        ...
        <button
          ...
          disabled={viewModel.isSubmitting ||
                        !!viewModel.isSocialSigningIn}
          onclick={() => viewModel.socialRegister("github")}
        >
      ...
      >
        <div class="form-control">
          ...
          <input
            ...
            class:input-error={viewModel.errors.displayName}
            placeholder={t.full_name_placeholder()}
            value={viewModel.form.displayName}
            oninput={(e) => {
                            viewModel.form.displayName = e.currentTarget.value;
                            viewModel.handleChange("displayName");
                        }}
            disabled={viewModel.isSubmitting ||
                            !!viewModel.isSocialSigningIn}
            autocomplete="name"
          >
          {#if viewModel.errors.displayName}
            <div class="label">
              <span class="label-text-alt text-error"> {viewModel.errors.displayName} </span>
            </div>
          {/if}
        </div>
