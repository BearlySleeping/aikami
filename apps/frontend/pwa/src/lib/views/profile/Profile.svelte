...
<BaseViewModelContainer {viewModel}>
  <div class="container mx-auto p-4">
    <div class="flex justify-between items-center mb-4">
      ...
      <button class="btn btn-secondary" onclick={() => viewModel.signOut()}>{t.sign_out()}</button>
    </div>

    {#if viewModel.user}
      <div class="mb-8">
        <h2 class="text-xl font-bold mb-2">{t.account_information()}</h2>
        ...
        {#each viewModel.characters as character (character.id)}
          ...
          >
            <div class="card-body">
              ...
              <div class="badge badge-secondary">Level {character.level}</div>
            </div>
          </div>
        {/each}
      </div>
