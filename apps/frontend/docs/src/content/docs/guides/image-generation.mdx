...
The **Image Generation Pipeline** produces AI-generated visuals (backgrounds, portraits, illustrations) using sd-server or ComfyUI, guided by configurable **style profiles** and automatic **contextual triggers**.
...
Image generation runs against **ComfyUI** or **sd-server** behind a single
abstraction, chosen by `PUBLIC_IMAGE_ENGINE` or auto-detected. See the
[image engine selection guide](image-engine-selection) for engine switching,
capability gating, and negative-prompt support.
