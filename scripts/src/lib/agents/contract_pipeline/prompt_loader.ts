...
export const loadReviewPrompt = (options: {
  ...
  return [
    ...
    '- `approve` — accept the run, stop here (no PR)',
    '- `approve_pr` — reconcile workspace → create PR to dev → wait for review',
    '- `approve_merge` — reconcile → PR → auto-merge ("send it")',
    '- `changes_applied` — code/contract edits made → re-verify',
    '- `reject` — block the run, keep workspace for diagnostics',
    '',
    'User shortcuts map as follows:',
    '- `/approve` or "looks good" → `approve`',
    '- `/ship` or "create PR" → `approve_pr`',
    '- `/merge` or "send it" → `approve_merge`',
    '- `/fix` or "I changed X" → `changes_applied`',
