...
const YOLO_INJECT = [
  ...
  'CodeRabbit automation exclusively — you do NOT edit code, run tests,',
  'or execute git commands yourself.',
  '',
  ...
  '',
  '**Y4 — Handoff**: Call `contract_review_decision` with `merge`.',
  '  Do NOT run `validate()` or any tests — CodeRabbit verified its autofix',
  '  commit on its own platform. The orchestrator handles final assembly.',
  '  Do NOT call `gh_merge_pr` yourself.',
  '',
  ...
  '| Y4: Handoff | `contract_review_decision` (merge) |',
].join('\n');
