...
async function main() {
  ...
  const projects = [
    ...
    { name: 'Hub', path: 'apps/frontend/hub', desc: 'SvelteKit SSR dashboard (Cloudflare Worker)' },
  ];
  ...
  const lines = [
    ...
    "## What We're Building",
    '',
    'Aikami is a monorepo application platform: SvelteKit Client (PWA) + Cloudflare-native backend + Bun runtime.',
    '',
    ...
    '## Tech Stack',
    '',
    '**Bun × SvelteKit 2 × Cloudflare × Moon × Biome**',
    '',
    '| Layer | Technology |',
    '|-------|-----------|',
    '| Runtime | Bun |',
    ...
    '- **Repository pattern** for D1/SQLite access',
