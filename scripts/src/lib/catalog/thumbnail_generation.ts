...
export const generateThumbnail = async (options: {
  ...
  // Non-LPC image: the asset IS the preview — re-encode as webp whole.
  const bytes = await sharp(sourcePath).webp({ quality: 90 }).toBuffer();
  return {
    hash: sha256Hex(new Uint8Array(bytes)),
    bytes: new Uint8Array(bytes),
...
export const runThumbnailPhase = async (options: {
  ...
  for (const entry of options.entries) {
    ...
    try {
      ...
      thumbnails.set(entry.tag, thumbnail);
    } catch (error) {
  ...
  // skip logic under the thumbnails/ prefix.
