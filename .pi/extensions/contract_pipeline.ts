...
export default function contractPipelineExtension(pi: ExtensionAPI): void {
  ...
  // 🔴 Worker-side reporting only. Outside a pipeline worker these four
  // actions can never be called correctly (they need CONTRACT_PIPELINE_ROLE,
  // _RUN_ID and _RESULT_PATH), so registering them in an ordinary session
  // would be pure prompt cost. The tool_call hook above stays active either
  // way — it is free.
  if (!isEnabled('contract_stage', isPipelineWorker())) {
    return;
  }

  registerNamespace(pi, {
    name: 'contract_stage',
    label: 'Contract Pipeline Stage',
    description: 'Worker-side reporting for an automated contract pipeline stage.',
    actions: [
      defineAction({
        action: 'complete',
        summary: 'Finish the current stage with a validated result artifact',
      ...
      defineAction({
        ...
        summary: 'Record the final user-informed review decision',
        ...
        parameters: Type.Object({
          ...
          details: Type.Optional(
            Type.String({
              maxLength: 20_000,
              description:
                "Only read on decision 'change' — passed to the implementer's next " +
                'attempt as additional context, alongside `summary`. Use this for ' +
                "anything too long or detailed for summary's 4096-char cap: an " +
                'AskClaude consultation you ran, root-cause notes, a suggested fix ' +
                'approach. If you consulted AskClaude or found something the ' +
                'implementer needs to see, put it here rather than fixing it yourself.',
            }),
          ),
        }),
        async execute(_toolCallId, params) {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Review decision recorded: ${params.decision}.${mergeWarning}`,
              },
            ],
            details: decision,
          };
      ...
      defineAction({
        action: 'log_failure',
        summary: 'Capture diagnostics for a failed worktree',
