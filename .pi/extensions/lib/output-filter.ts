...
export function parseMoonProjects(raw: string): LightProject[] | null {
  if (!raw || raw.length > MOON_JSON_MAX_RAW) return null
...
export function formatProjectList(projects: LightProject[]): string {
  ...
  const parts: string[] = []
  if (apps.length > 0) parts.push(`**Apps (${apps.length})**: ${apps.map((p) => p.id).join(", ")}`)
  if (libs.length > 0) parts.push(`**Libs (${libs.length})**: ${libs.map((p) => p.id).join(", ")}`)
  if (other.length > 0) parts.push(`**Other (${other.length})**: ${other.map((p) => p.id).join(", ")}`)
...
export function extractAffectedIds(raw: string): string[] {
  ...
  if (projects) return projects.map((p) => p.id)
...
export function filterFixOutput(raw: string): string {
  ...
  const kept = lines.filter((l) => {
    if (shouldStrip(l)) return false
    if (l.includes("Fixed") || l.includes("applied") || l.includes("error")) return true
    if (l.includes("Checked") || l.includes("files")) return true
    return shouldKeep(l)
  })

  if (kept.length === 0) return "✅ No issues fixed (clean)"
...
export function filterTypecheckOutput(raw: string): string {
  ...
  if (errors.length === 0 && warnings.length === 0) {
    ...
    if (nonEmpty.length === 0) return "✅ Typecheck passed (no errors)"
  ...
  if (errors.length > 0) {
    ...
    if (errors.length > 40) parts.push(`... and ${errors.length - 40} more`)
  }
...
export function filterTestOutput(raw: string): string {
  ...
  const kept = lines.filter((l) => {
    ...
    // Keep summary lines
    if (/(\d+)\s+(passed|failed|skipped|tests)/i.test(trimmed)) return true
    // Keep error lines
    if (shouldKeep(l)) return true
    // Keep test names
    if (/^✓|^✗|^✘|^PASS|^FAIL|^\d+\.\s/.test(trimmed)) return true
    return false
  })

  if (kept.length === 0) return "No test output captured."
...
export function guessTaskType(target: string): "fix" | "typecheck" | "build" | "test" | "other" {
  ...
  if (task === "fix" || task === "lint") return "fix"
  if (task === "typecheck" || task === "tc") return "typecheck"
