...
- 🔴 **Every file you touch must go through `contract_stage` action `validate` before it is pushed.** It is the only checked commit path (all others pass `--no-verify`), and `gh_pr create` is blocked until a verdict exists for the exact commit on the remote. A RED verdict also blocks PR creation — to publish it deliberately, get the user's explicit permission and record it with `contract_stage` action `authorizePublication` (YOLO records one automatically); the authorization is bound to that exact commit and voids on any new commit. Re-running one `moon run <task>` you happen to know about is not a substitute — that is precisely how C-484 put a `client:format` failure on CI.
