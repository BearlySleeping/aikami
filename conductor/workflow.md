...
6. **Format, Lint, and Check:**
   - After refactoring, you MUST run the project's formatting, linting, and static analysis checks to ensure code quality and adherence to standards.
   - For a single project (e.g., `pwa`), run the checks for that specific project: `deno task moon pwa:check`
   - For multiple projects, run the checks for all projects: `deno task check`
