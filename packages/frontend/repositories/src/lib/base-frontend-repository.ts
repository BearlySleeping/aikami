...
export class FrontendRepository<T extends RepositoryType> extends BaseRepository<T>
	...
	async getDocument(
		...
		const [{ getDoc }, documentReference] = await Promise.all([
			this._getDatabase(),
			this.getDocumentReference<z.infer<T['data']>>(
				getDocumentPathArguments,
			),
		]);
	...
	async getDocumentsByCollection(
		...
		const [{ getDocs }, collectionReference] = await Promise.all([
			this._getDatabase(),
			this.getCollectionReference<z.infer<T['data']>>(
	...
	async commit(batchCommands: BatchCommand[]): Promise<void> {
		...
		const addToBatchCommand = async ({
			data,
			documentPathArgument,
			type,
		}: BatchCommand) => {
			...
			for (const query of queries) {
				unsubscribes.push(
					onSnapshot(
						query,
						async (querySnapshot: QuerySnapshot<z.infer<T['data']>>) => {
							const newDocuments = await this.parseDocuments(
								querySnapshot.docs,
							);
	...
	async getRawQuery(options: {
		...
		for (const { field, operator, value } of filters) {
			constraints.push(
				where(field === 'id' ? documentId() : field, operator, value),
			);
		}
	...
	async getDocumentStream(
		...
		const { onSnapshot } = await this._getDatabase();
		const documentReference = await this.getDocumentReference<z.infer<T['data']>>(
			getDocumentPathArgument,
		);
	...
	>(
		documentArgument: T['getDocumentPathArgument'],
	): Promise<DocumentReference<Document>> {
		const { doc, firestore } = await this._getDatabase();

		return doc(
			firestore,
			this.getDocumentPath(documentArgument),
		) as DocumentReference<Document>;
	}
	...
	async updateDocument({
		...
		const [{ serverTimestamp, setDoc, updateDoc }, documentReference] = await Promise.all([
			this._getDatabase(),
			this.getDocumentReference<Partial<z.infer<T['updateData']>>>(
				getDocumentPathArgument,
			),
		]);
	...
	async getRawCollectionReference(
		collectionPath: string,
	): Promise<CollectionReference> {
	...
	async getCollectionReference<
		Document extends z.infer<T['data']> | z.infer<T['createData']>,
	>(
		getCollectionPathArgument: T['getCollectionPathArgument'],
	...
	async getQuery(
		...
		if (startAfterParameter) {
			...
			if (isCustomTimestamp(startAfterParameter)) {
				...
				const millis = startAfterParameter._seconds * 1000 +
					startAfterParameter._nanoseconds / 1000000;
		...
		return query<z.infer<T['data']>, z.infer<T['data']>>(
			...
			options.collectionGroupName
				? await this.getCollectionGroupReference(
					options.collectionGroupName,
				)
				: await this.getCollectionReference(
					options.getCollectionPathArgument,
				),
			...constraints,
		);
	}
