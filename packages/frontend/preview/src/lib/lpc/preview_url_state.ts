// packages/frontend/preview/src/lib/lpc/preview_url_state.ts
//
// LPC preview URL state serialisation — bidirectional mapping between
// URLSearchParams and LPC layer/animation state.
// Moved from apps/frontend/client/src/lib/data/lpc_url_config.ts (C-445).
//
// The encoding uses compact positional keys:
...
// ---------------------------------------------------------------------------
// Serialise: LpcPreviewState → URLSearchParams
// ---------------------------------------------------------------------------

/**
 * Encodes LPC preview state into URLSearchParams.
 ...
 */
export const encodeLpcPreviewState = (state: LpcPreviewState): URLSearchParams => {
...
// ---------------------------------------------------------------------------
// Deserialise: URLSearchParams → LpcPreviewState
// ---------------------------------------------------------------------------

/**
 * Decodes URLSearchParams into LPC preview state with defaults for absent keys.
 *
 * @param params - URLSearchParams from the current page URL.
 * @returns Parsed LpcPreviewState with defaults for missing values.
 */
export const decodeLpcPreviewState = (params: URLSearchParams): LpcPreviewState => {
