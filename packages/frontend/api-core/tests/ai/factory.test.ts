...
describe('createAiClient factory', () => {
  ...
  it('throws when openai provider is missing apiClient', async () => {
    expect(createAiClient('openai')).rejects.toThrow('requires an apiClient');
  });

  it('throws when gemini provider is missing apiClient', async () => {
    expect(createAiClient('gemini')).rejects.toThrow('requires an apiClient');
  });

  it('throws when comfyui provider is missing workflowId', async () => {
    expect(createAiClient('comfyui', { comfyui: {} })).rejects.toThrow('requires a workflowId');
  });

  it('throws for unknown provider', async () => {
    expect(createAiClient('unknown' as never)).rejects.toThrow('Unsupported AI provider');
  });
});
