...
export type PathParameters<T extends RouteName> = ArgumentTypes<
  AllRoutes[T]['getPath']
>
...
export const isPublicPage = (route: string): boolean => {
  ...
  if (routeOptions) {
    return (
      (routeOptions.type) === 'public' ||
      routeOptions.type === 'unauthenticated'
    )
  }
  return false
}
...
export const toRoutePathFromURL = (url: URL): RouteName | undefined => {
  ...
  for (
    const [routeName, routeOptions] of Object.entries(routes) as [string, AllRoutes[RouteName]][]
  ) {
...
export const toRoutePathFromRouteId = (
  routeId: string,
): RouteName | undefined => {
  ...
  for (
    const [routeName, routeOptions] of Object.entries(routes) as [
      string,
      AllRoutes[RouteName],
    ][]
  ) {
...
export const getRoutePath = <T extends RouteName>(options: {
  ...
  // @ts-ignore pathParameters can be undefined for the entire router, or be defined for some on another router, in order to
...
export const toNavigateToAppHref = (options: {
  defaultHref: string
  url: URL
}): string => {
