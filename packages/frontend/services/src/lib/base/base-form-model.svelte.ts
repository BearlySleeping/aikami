...
} from './base-view-model.svelte.ts'

export type BaseFormViewModelOptions<
...
export type BaseFormViewModelInterface<
  FormSchema extends ZodType<z.infer<typeof CoreFormSchema>>,
> = {
  /** Form data */
...
> extends BaseViewModel<Options> implements BaseFormViewModelInterface<FormSchema> {
  ...
  protected _errors = $state<
    Partial<Record<keyof z.infer<FormSchema>, string>>
  >({})

  private readonly _initialValues: z.infer<FormSchema>
  private readonly _getInitialValues:
    | (() => Promise<z.infer<FormSchema>>)
    | undefined

  private readonly _schema: FormSchema
  private readonly _onSubmitCallback: (
    values: z.infer<FormSchema>,
  ) => Promise<void>
  ...
  async handleChange(
    key: Exclude<keyof z.infer<FormSchema>, 'id'>,
  ): Promise<void> {
  ...
  errors = $derived(
    (() => {
      const errors: Partial<
        Record<keyof z.infer<FormSchema>, string>
      > = {}
      ...
      for (const [field, errorMessage] of Object.entries(this._errors)) {
        ...
        if (!localizedMessage) {
          this.error(
            'error:missing-localized-error-message',
            message,
          )
  ...
  private async _validateAllFields(): Promise<
    | [true]
    | [false, Partial<Record<keyof z.infer<FormSchema>, string>>]
  > {
    const formValue = this.form
