...
> = {
  set<Key extends keyof PreferenceRecords>(key: Key, value?: PreferenceRecords[Key]): Promise<void>;

  get<Key extends keyof PreferenceRecords>(key: Key): Promise<PreferenceRecords[Key] | undefined>;

  delete<Key extends keyof PreferenceRecords>(key: Key): Promise<void>;

  clear(): Promise<void>;

  entries(): Promise<[keyof PreferenceRecords, PreferenceRecords[keyof PreferenceRecords]][]>;
};
  ...
  implements CorePreferenceProviderInterface<PreferenceRecords>
{
  ...
  > {
    if (CorePreferenceProviderService._preferenceProvider) {
      return CorePreferenceProviderService._preferenceProvider as CorePreferenceProviderInterface<PreferenceRecords>;
    }
    if (
