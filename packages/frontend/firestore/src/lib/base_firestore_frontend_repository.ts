// packages/frontend/firestore/src/lib/base_frontend_repository.ts
...
export type { FirestoreRepositoryType, QueryFilter };
...
export type FirestoreFrontendRepositoryInterface<T extends FirestoreRepositoryType> = {
...
export class FirestoreFrontendRepository<T extends FirestoreRepositoryType>
  extends BaseFirestoreRepository<T>
  implements FirestoreFrontendRepositoryInterface<T>
{
  ...
  private async _getDatabase(): Promise<Database> {
    if (FirestoreFrontendRepository._database) {
      return FirestoreFrontendRepository._database;
    }
    ...
    FirestoreFrontendRepository._database = await import('@aikami/frontend/configs/firestore');
    return FirestoreFrontendRepository._database;
  }
}
