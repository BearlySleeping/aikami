...
export const NpcSchema = CoreSchema.extend(NpcSheetSchema.shape).extend({
  avatarUrl: z.url().describe('URL of the character image').optional(),
})

export const NpcCreateSchema = NpcSchema
  .omit(CoreOmitSchema)
  .extend(
    CoreCreateSchema.shape,
  )

export const NpcUpdateSchema = NpcSchema
  .extend(getDeletableFields(NpcSchema)).omit(CoreOmitSchema)
