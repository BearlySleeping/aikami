...
export const ImageGenerationProviderSchema = z.enum(['dalle', 'stable-diffusion', 'comfyui']);
...
export const ImageGenerationRequestSchema = z.object({
  ...
  quality: z.enum(['standard', 'hd', 'ultra']).optional().describe('Generation quality'),
  seed: z.number().optional().describe('Seed for reproducible results'),
  steps: z.number().min(1).max(150).optional().describe('Number of diffusion steps'),
  cfgScale: z.number().min(1).max(20).optional().describe('CFG scale for diffusion'),
