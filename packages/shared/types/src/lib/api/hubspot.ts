...
export type HubSpotAction = 'IFRAME' | 'CONFIRMATION_ACTION_HOOK' | 'ACTION_HOOK';

export type HubSpotActionData<T extends HubSpotAction = HubSpotAction> = {
  type: T;
} & (T extends 'IFRAME'
  ? HubSpotIFrameActionData
  : T extends 'CONFIRMATION_ACTION_HOOK'
    ? HubSpotConfirmActionData
    : T extends 'ACTION_HOOK'
      ? HubSpotActionHookData
      : never);
