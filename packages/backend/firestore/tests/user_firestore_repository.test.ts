// packages/backend/firestore/tests/user-repository.test.ts
...
} from '../src/lib/user_firestore_repository';
...
/**
 * Run the full UserFirestoreRepository test battery against a {@link BaseFirestoreService}
 * implementation.  Call this once for the mock and once for the real service.
 */
const runRepositoryTests = (
  label: string,
  createDb: () => BaseFirestoreService,
  setup?: (db: BaseFirestoreService) => void | Promise<void>,
  teardown?: (db: BaseFirestoreService) => void | Promise<void>,
): void => {
  describe(`UserFirestoreRepository — ${label}`, () => {
    let db: BaseFirestoreService;
    let repo: UserFirestoreRepository;

    beforeEach(async () => {
      db = createDb();
      repo = new UserFirestoreRepository(db);
...
// =========================================================================
// Unit tests — MockFirestoreService (fast, always run)
// =========================================================================
