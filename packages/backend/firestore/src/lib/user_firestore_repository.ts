// packages/backend/firestore/src/lib/user_firestore_repository.ts
import type { BaseFirestoreService, QueryOptions } from './base_firestore_service.ts';
...
/**
 * Example repository demonstrating the {@link BaseFirestoreService} abstraction.
 *
...
export class UserFirestoreRepository {
  ...
  constructor(private readonly db: BaseFirestoreService) {
    if (!db) {
      throw new Error('UserFirestoreRepository requires a BaseFirestoreService instance.');
    }
