...
export type UserFirestoreRepositoryInterface =
  FirestoreBackendRepositoryInterface<UserFirestoreRepositoryType>;

export const userFirestoreRepository: UserFirestoreRepositoryInterface =
  new FirestoreBackendRepository<UserFirestoreRepositoryType>({
...
export const getUserData = async (uid: string): Promise<undefined | UserData> => {
  try {
    return await userFirestoreRepository.getDocument({ uid });
  } catch (error) {
...
export const updateUserData = async (
  ...
  try {
    ...
    await userFirestoreRepository.updateDocument({
...
export const setUserData = async (uid: string, userData: UserCreateData): Promise<void> => {
  try {
    await userFirestoreRepository.setDocument({
      getDocumentPathArgument: { uid },
      setData: userData,
    });
  } catch (error) {
...
export const deleteUserData = async (uid: string): Promise<boolean> => {
  try {
    await userFirestoreRepository.deleteDocument({
      uid,
    });
    return true;
  } catch (error) {
...
export const getUserByEmail = async (email: string): Promise<undefined | UserData> => {
  try {
    const users = await userFirestoreRepository.getDocumentsByQuery({
      filters: [
        {
          field: 'email',
          operator: '==',
