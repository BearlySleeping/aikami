// packages/backend/firestore/src/lib/base_firestore_backend_repository.ts

import { backendEnv } from '@aikami/backend/configs/environment';
import { documentId, getFirestore, serverTimestamp } from '@aikami/backend/configs/firestore';
import { getBatch } from '@aikami/backend/utils/batch';
import type { FirestoreRepositoryType, GetQueryOptions, QueryFilter } from '@aikami/types';
import {
  BaseFirestoreRepository,
  type BaseFirestoreRepositoryInterface,
  type BaseFirestoreRepositoryOptions,
} from '@aikami/utils';
import type {
  CollectionReference,
  DocumentReference,
...
export type FirestoreBackendRepositoryInterface<T extends FirestoreRepositoryType> =
  BaseFirestoreRepositoryInterface<T> & {
...
export class FirestoreBackendRepository<T extends FirestoreRepositoryType>
  extends BaseFirestoreRepository<T>
  implements FirestoreBackendRepositoryInterface<T>
{
  private static _database?: Firestore;

  constructor(options: BaseFirestoreRepositoryOptions<T>) {
    super({
      ...options,
  ...
  private async _getDatabase(): Promise<Firestore> {
    if (FirestoreBackendRepository._database) {
      return FirestoreBackendRepository._database;
    }
    await Promise.resolve();
    FirestoreBackendRepository._database = getFirestore();
