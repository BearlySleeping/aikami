...
//
//   1. createWorkerObjectStore(env.R2_BUCKET) — for the hub's Cloudflare
//      Worker runtime (uses the native R2Bucket binding).
//   2. createS3ObjectStore(config) — for the catalog publish pipeline
//      (uses Bun.S3Client against the R2 S3-compatible endpoint).
//
...
export type ObjectStore = {
   ...
   */
  put<Params extends Record<string, string>>(
    spec: KeySpec<Params>,
    params: Params,
    body: ArrayBuffer,
    options?: PutOptions,
   ...
   */
  get<Params extends Record<string, string>>(
    spec: KeySpec<Params>,
    params: Params,
   ...
   */
  delete<Params extends Record<string, string>>(
    spec: KeySpec<Params>,
    params: Params,
  ): Promise<void>;
   ...
   */
  list<Params extends Record<string, string>>(
    spec: KeySpec<Params>,
    prefixParams: Params,
  ): Promise<string[]>;
};
...
/**
 * Create an ObjectStore backed by a Cloudflare Worker R2Bucket binding.
 *
 * @param bucket - The R2Bucket binding (e.g. env.SAVES_BUCKET).
 */
export const createWorkerObjectStore = (
  bucket: import('@cloudflare/workers-types').R2Bucket,
): ObjectStore => {
  const toKey = <Params extends Record<string, string>>(
    spec: KeySpec<Params>,
    params: Params,
  ): string => spec.build(params);

  return {
    async put(spec, params, body, options) {
      const key = toKey(spec, params);
 ...
 *
 * @param s3 - A configured Bun.S3Client instance.
 */
export const createS3ObjectStore = (s3: Bun.S3Client): ObjectStore => {
  return {
    async put(spec, params, body, options) {
