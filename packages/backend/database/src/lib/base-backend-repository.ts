...
export type BackendRepositoryInterface<T extends RepositoryType> =
  ...
  & {
    ...
    ): QueryFilter<z.infer<T['data']>>[]
  }

export class BackendRepository<T extends RepositoryType> extends BaseRepository<T>
  ...
  constructor(options: BaseRepositoryOptions<T>) {
    super({
      ...options,
      parseLevel: options.parseLevel ??
        (getEnvironmentValue('PARSE_LEVEL', true) as
          | ParseLevel
          | undefined),
    })
  }
  ...
  async getDocument(
    ...
    const documentReference = await this.getDocumentReference<
      z.infer<T['data']>
  ...
  async getNewDocumentId(
    ...
    const newDocumentId = (
      await this.getCollectionReference(getCollectionPathArgument)
    ).doc().id
  ...
  async updateDocument({
    ...
    if (options?.merge) {
      ...
      await documentReference.set(
        data as PartialWithFieldValue<z.infer<T['data']>>,
        { merge: true },
      )
    } else {
  ...
  private async _getQuery(
    ...
    const database = await this._getDatabase()
    let query =
      (collectionGroupName ? database.collectionGroup(collectionGroupName) : database.collection(
        this.getCollectionPath(getCollectionPathArgument),
      )) as unknown as Query<z.infer<T['data']>>
