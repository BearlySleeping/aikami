...
4. **Firestore isolation theoretical**: The Client uses a fixed project ID (`aikami-staging`), so client-side Firestore requests always hit the same namespace. Worker isolation is primarily for Auth state and teardown cleanup. Full Firestore isolation would require the Client to accept a dynamic project ID (out of scope).
