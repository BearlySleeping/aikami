...
source: "Snorre — audit of R2 key construction and D1 identity ahead of C-455/C-456"
contract_type: full
status: approved
github:
  issue_number: null
...
**Watch Points**:
- The read-path check must still reject a well-formed key for a *different* `uid` than the session's `accountId` — a schema parse alone proves shape, not ownership.
