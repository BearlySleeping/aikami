...
github:
  ...
  pr_url: "https://github.com/BearlySleeping/aikami/pull/142"
  pr_number: 142
...
### Summary
Implemented C-390 for real — the previous `implemented` status and report in
this file were fabricated (commit `f6504885` appended report text only; no
`compose.yaml`, `stack/` fetcher, or workflows existed anywhere in the repo).
This pass delivered the full publishable topology: `compose.yaml` with
profile-selected modalities and 6 backend override files (cpu/cuda/rocm/
vulkan/intel/musa), all upstream images digest-pinned with digests resolved
live from the registries (2026-08-14), the checksum-verified profile-scoped
model fetcher with 14 unit tests (AC-6/AC-7), the pinned model catalog with
real HF/GitHub SHA-256 digests, the CPU sd-server image (upstream publishes
no CPU tag), the GHCR publish workflow (AC-5/AC-9 checks + anonymous-pull
verification), the weekly digest-bump job, and deletion of the Ultimate
container + lite compose. Ports now match `development_ports.ts` (text 11434,
image 8188, voice 8089, stt 8087, web 5274). Live-verified the real text
engine (llama.cpp image + real Qwen2.5-1.5B model → `/health` 200 + a real
completion), the real voice engine (built sherpa image + real Kokoro model →
`/v1/audio/speech` returned a valid 114KB WAV), and the real web client
(HTTP 200 + two different `config.json` mounts served with no rebuild).
Docs page `run-locally.mdx` written; `configure-local-engines.mdx` ports
updated; README rewritten with the macOS native path.

### AC Status
| AC | Status | Notes |
|---|---|---|
| AC-1 | ✅ | Every upstream `image:` resolves (verified live via `docker manifest inspect` for llama.cpp×7, sd.cpp×5, ollama, comfyui) and is `@sha256:` pinned; `check.sh` AC-1 loop + unit pin assertions |
| AC-1b | ✅ | Per-backend render assertions for cpu/cuda/rocm/vulkan/intel/musa pass; rocm → `server-rocm` text + `master-vulkan` image (no ROCm sd image published) |
| AC-2 | ✅ | `--profile <modality> config --services` verified: fetcher + exactly the engine; web alone when web-only |
| AC-3 | ✅ | Base render → CPU images with no reservation; CUDA render → `server-cuda` + NVIDIA device reservation |
| AC-4 | ✅ (live text/voice/web) | Real image + real model live-probed: llama.cpp `/health` 200 + completion, sherpa `/v1/audio/speech` 200 + valid WAV, client HTTP 200 + config swap. Full-stack probes wired into `check.sh` under `LOCAL_STACK_LIVE=1` |
| AC-5 | ✅ (workflow) | Layer-inspection step in `publish-local-stack.yml` (no weights, 250MB budget) |
| AC-6 | ✅ | 14 unit tests: resume (Range), idempotent, corrupt re-fetch, profile scope, non-fatal per-modality; real 4.6GB partial download resumed |
| AC-7 | ✅ | Unit tests: SD 1.5 skipped without `AIKAMI_ACCEPT_LICENSES` (prints licence+URL), downloads once accepted |
| AC-8 | ⚠️ | Degraded-mode design in place (health checks name the missing model, engines independent); offline boot is a documented manual check in README |
| AC-9 | ✅ (live) | Two `config.json` mounts (10.1.1.1 / 10.2.2.2) against ONE built client image both served HTTP 200 + correct config with no rebuild; same test in `publish-local-stack.yml` |
...
| AC-11 | ✅ | `ports.test.ts` against `development_ports.ts`: text 11434, image 8188, voice 8089, stt 8087, web 5274; no 8080; all host publishes bind 127.0.0.1 |
| AC-12 | ✅ (design + check) | README states no-Metal-passthrough plainly and directs to `bin/run-native-*.sh` (now on table ports); `check.sh` verifies native launcher ports on all platforms |
| AC-13 | ✅ | `compose.models-path.yaml` renders a `MODELS_PATH` bind mount in place of the named volume (verified in `check.sh`); original tree untouched by design |

### Files Created
| File | Purpose |
|---|---|
| `apps/backend/local-stack/compose.yaml` | Base topology: profiles, health checks, depends_on ordering, digest pins, model-fetcher one-shot, ollama/comfyui advanced profiles |
| `apps/backend/local-stack/compose.cpu.yaml` | Explicit CPU backend override |
...
| `apps/backend/local-stack/compose.models-path.yaml` | AC-13: `MODELS_PATH` bind-mount alternative to the named volume |
| `apps/backend/local-stack/.env.example` | `.env` contract: COMPOSE_PROFILES / COMPOSE_FILE / TEXT_MODEL / IMAGE_MODEL / AIKAMI_ACCEPT_LICENSES / ENABLE_STT / MODELS_PATH |
| `apps/backend/local-stack/stack/fetch_models.ts` | Checksum-verified, resumable (Range), profile-scoped, ack-gated, non-fatal fetcher (Bun, dependency-free) |
| `apps/backend/local-stack/stack/fetch_models.test.ts` | 14 unit tests (AC-6/AC-7 incl. resume, corrupt, profile scope, ack gate, archive) |
| `apps/backend/local-stack/stack/ports.test.ts` | AC-11 port-table conformance |
| `apps/backend/local-stack/stack/repo_structure.test.ts` | AC-10 removed-artifacts + compose/manifest invariants |
| `apps/backend/local-stack/stack/models.manifest.json` | Pinned catalog: real sizes + SHA-256 for 7 entries (3 text, 2 image incl. ack-gated SD 1.5, TTS, STT) |
| `apps/backend/local-stack/docker/sd-server/Dockerfile.cpu` | CPU sd-server build (pinned upstream revision de298c2) |
| `.github/workflows/publish-local-stack.yml` | GHCR publish: voice / sd-server:cpu / client + AC-1/AC-5/AC-9 verification + anonymous-pull check |
...
| File | Change |
|---|---|
| `packages/shared/constants/src/lib/development_ports.ts` | Added `stt` (emulator 8087, staging 8086, production 8090) to all port tables |
| `apps/backend/local-stack/Dockerfile.client` | Port 3000 → 5274; runtime `config.json` mount documented (AC-9); non-root nginx fixed (cache/pid paths, `/tmp/nginx.pid`) — the previous image could not start |
| `apps/backend/local-stack/docker/client/nginx.conf` | Listen 3000 → 5274 |
| `apps/backend/local-stack/docker/voice/Dockerfile.sherpa` | Ports 6006/6007 → 8089/8087; EXPOSE updated |
| `apps/backend/local-stack/docker/voice/entrypoint.sh` | TTS/STT port defaults; model paths from the shared volume; STT gated by ENABLE_STT |
| `apps/backend/local-stack/docker/voice/tts_server.py` | Default port 6006 → 8089 |
...
| `apps/backend/local-stack/scripts/emit_config.sh` | Defaults move to table ports (11434/8188/8089/8087); emits `voice.stt.url` |
| `apps/backend/local-stack/scripts/check.sh` | Rewritten: unit tests + AC-1/1b/2/3/9/11/13 assertions + `LOCAL_STACK_LIVE=1` AC-4 live probes |
| `apps/backend/local-stack/moon.yml` | Tasks updated (up/up-cpu/up-cuda, fetch-models); removed lite/ultimate; dependsOn constants; typecheck via inherited global task |
| `apps/backend/local-stack/package.json` | Scripts updated (lint matrix, fetch-models, build:sd-server:cpu); `@aikami/constants` dep |
| `apps/backend/local-stack/tsconfig.json` | Include `stack/**`; paths for constants/types |
| `apps/backend/local-stack/README.md` | Full rewrite: quick start, backends, models, macOS native path, smoke tests, container security |
| `apps/frontend/docs/src/content/docs/guides/configure-local-engines.mdx` | Example config ports 8080/6006 → 11434/8089; removed deleted client-server reference |

### Deviations from Spec
- **Model catalog sources**: the contract's recommended Qwen3 GGUF repos are
  HF-gated (401). The manifest pins the accessible Apache-2.0 mirrors
  (`bartowski/` Qwen2.5-1.5B/7B, Mistral-Nemo-12B) with real, verified
  digests instead; SD 1.5 remains the acknowledgement-gated OpenRAIL-M entry.
  Catalog choice, not an AC change.
- **AC-4/AC-8 live checks**: text, voice, and web were live-verified here
  against real images/models. Image (sd-server) boot requires a large model
  download + GPU/CPU inference environment; the probe is wired into
  `check.sh` under `LOCAL_STACK_LIVE=1` and the publish workflow. AC-8
  (offline boot) is a documented manual check per the Evidence Matrix. No AC
  text changed.
- **`server-cuda13`**: the contract table lists it; no separate override file
  ships (the README documents swapping the CUDA tag manually for CUDA-13
  hosts). The digest-bump job tracks it.
...
### Test Results
- Unit: 33/33 PASS (14 fetcher + 8 ports + 11 repo-structure; 0 failures)
- check.sh --static: 47/47 PASS (0 failures)
- check.sh LOCAL_STACK_LIVE=1: 51 PASS, 4 AC-4 live-probe FAILs (expected —
  containers cleaned up after live verification; probes passed during the
  live run below)
- E2E: N/A (no application UI in this contract)
- Visual: docs page built and served HTTP 200 (screenshots unavailable —
  herdr client/site services fail with "Bun is not defined", an environment
  issue; static serving of the built docs verified by HTTP)
- Baseline: constants suite 114 tests green; 0 pre-existing, 0 new failures
- Live smoke (production path): text `/health` 200 + real completion
  "LOCAL-STACK-LIVE-OK"; voice `/v1/audio/speech` 200 + 114KB valid WAV;
  client HTTP 200 + two config mounts served with no rebuild
- `validate({test:true})`: 4/4 projects passed (fix + typecheck + build + test)
