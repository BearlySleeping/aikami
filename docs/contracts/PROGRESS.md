# Contract Implementation Progress

## Status Summary (Auto-generated: 2026-09-14)

**284 active (0 without contract file), 119 archived, 0 duplicates**

### Active Contracts

| Contract | Name | Status | Promotion | Version | Type |
|----------|------|--------|-----------|---------|------|
| C-011 | Blackbox Testing | 🏁 completed | — | v1 | full |
| C-014 | Database Abstraction And Dataconnect | 🏁 completed | — | v1 | full |
| C-015 | Ai Service Abstraction | ⏳ not_started | — | v1 | full |
| C-031 | Adapter Static And Hosting | ⏳ not_started | — | v1 | full |
| C-056 | Hybrid Text Gateway | ⏳ not_started | — | v1 | full |
| C-117 | Ecs Snapshot Serializer | ⏳ not_started | — | v1 | full |
| C-118 | Save Load Ui Wiring | ⏳ not_started | — | v1 | full |
| C-119 | Routing And Layout Simplification | ⏳ not_started | — | v1 | full |
| C-120 | View Folder Restructure | ⏳ not_started | — | v1 | full |
| C-121 | Start Menu And Auth | ⏳ not_started | — | v1 | full |
| C-123 | Character Creation Flow | ⏳ not_started | — | v1 | full |
| C-124 | Game Engine Initialization | ⏳ not_started | — | v1 | full |
| C-125 | Game Ui Overlay Architecture | ⏳ not_started | — | v1 | full |
| C-127 | Settings Menu Refactor | ⏳ not_started | — | v1 | full |
| C-128 | Dialogue Overlay And Ai Chat | ⏳ not_started | — | v1 | full |
| C-129 | Dialogue Ai Integration Polish | 🏁 completed | — | v1 | full |
| C-132 | Save Load System | 🏁 completed | 🔗 integrated | v1 | full |
| C-133 | Flexible Provider Onboarding | 🏁 completed | — | v1 | full |
| C-134 | Inline Provider Setup | 🏁 completed | — | v1 | full |
| C-135 | Tilemap Environment Parsing | ⏳ not_started | — | v1 | full |
| C-136 | Entity Prop Spawner | ⏳ not_started | — | v1 | full |
| C-138 | Map Transitions | ⏳ not_started | — | v1 | full |
| C-140 | Game Mode System | ⏳ not_started | — | v1 | full |
| C-141 | Npc Interaction Trigger | ⏳ not_started | — | v1 | full |
| C-142 | Inventory Item Pickups | ⏳ not_started | — | v1 | full |
| C-143 | Quest Log And Test Fixes | ⏳ not_started | — | v1 | full |
| C-144 | Combat Encounter Integration | ⏳ not_started | — | v1 | full |
| C-149 | Combat Gatekeeping | ⏳ not_started | — | v1 | full |
| C-152 | End To End Boot Flow | 🏁 completed | — | v1 | full |
| C-153 | Character Dashboard Equipment | 🏁 completed | — | v1 | full |
| C-154 | Ai Vendors Economy | 🏁 completed | — | v1 | full |
| C-155 | Autosave Memory Hardening | 🏁 completed | — | v1 | full |
| C-156 | Tauri Production Release | 🏁 completed | — | v1 | full |
| C-157 | Dialogue Skill Checks | 🏁 completed | — | v1 | full |
| C-158 | Lpc Avatar Integration | 🏁 completed | — | v1 | full |
| C-159 | Demo Happy Path E2e | 🏁 completed | — | v1 | full |
| C-161 | Spatial Ui Camera | 🏁 completed | — | v1 | full |
| C-162 | Bg3 Action Menu Dice | 🏁 completed | — | v1 | full |
| C-163 | Visceral Feedback Juice | 🏁 completed | — | v1 | full |
| C-164 | Combat Split Screen Layout | 🏁 completed | — | v1 | full |
| C-168 | Pixijs Asset Pipeline Fix | ⏳ not_started | — | v1 | full |
| C-173 | Ecs Spatial Hash Grid | 🏁 completed | — | v1 | full |
| C-175 | Llm Jton Map Pipeline | 🏁 completed | — | v1 | full |
| C-180 | Engine Stability Harness | 🏁 completed | — | v1 | full |
| C-181 | Ai Visual Testing Framework | 🏁 completed | 🔗 integrated | v1 | full |
| C-182 | Visual Framework Polish | 🏁 completed | 🔗 integrated | v1 | full |
| C-183 | E2e Worker Isolation | 🏁 completed | 🔗 integrated | v1 | full |
| C-190 | Ecs Spatial Vision Systems | 🏁 completed | 🚀 release_verified | v1 | full |
| C-191 | Goap Bitmask Scheduler | 🏁 completed | 🔗 integrated | v1 | full |
| C-192 | Ecs Time Sliced Jps Pathfinder | 🏁 completed | 🔗 integrated | v1 | full |
| C-193 | Client Tool Streaming Orchestrator | 🏁 completed | 🔗 integrated | v1 | full |
| C-194 | Ecs Offscreen Macro Simulation | 🏁 completed | 🔗 integrated | v1 | full |
| C-195 | Ecs String Registry Hydration | 🏁 completed | 🔗 integrated | v1 | full |
| C-196 | Ecs Emergent World Integration | 🏁 completed | 🔗 integrated | v1 | full |
| C-197 | Ecs Goap Combat Tactics | ❓ ✅ completed | — | v1 | full |
| C-198 | Dev Sandbox Polish And Zoning | 🏁 completed | 🔗 integrated | v1 | full |
| C-199 | Visual Camera Alignment | 🏁 completed | 🔗 integrated | v1 | full |
| C-200 | Visual Pipeline Optimization | ❓ ✅ completed | 🔗 integrated | v1 | full |
| C-201 | Hide Dev Overlays On Screenshot | 🏁 completed | 🧪 sandbox | v1 | full |
| C-202 | Provider Settings Ux Overhaul | 🏁 completed | 🔗 integrated | v1 | full |
| C-203 | Local First Turso Sync | 🏁 completed | 🧪 sandbox | v1 | full |
| C-204 | Auxiliary Systems Config | 🏁 completed | 🧪 sandbox | v1 | full |
| C-210 | Webgpu Tilemap Integration | 🏁 completed | 🔗 integrated | v1 | full |
| C-211 | Realtime Tts Streaming Pipeline | 🏁 completed | 🔗 integrated | v1 | full |
| C-212 | Party Follow System | 🏁 completed | 🧪 sandbox | v1 | full |
| C-213 | Environment Time System | ⏳ not_started | — | v1 | full |
| C-214 | Engine And Api Consolidation | ⏳ not_started | — | v1 | full |
| C-215 | Data And Terminology Unification | ⏳ not_started | — | v1 | full |
| C-216 | Documentation Overhaul | ⏳ not_started | — | v1 | full |
| C-217 | E2e Stabilisation | ⏳ not_started | — | v1 | full |
| C-218 | E2e Logic Ui Resolution | ⏳ not_started | — | v1 | full |
| C-219 | Ui Component System Refactor | ❓ ✅ completed | 🧪 sandbox | v1 | full |
| C-220 | Ui Skills And Theming | ❓ ✅ completed | 🧪 sandbox | v1 | full |
| C-221 | Modal Dialog Encapsulation | ❓ ✅ completed | 🧪 sandbox | v1 | full |
| C-230 | Provider Connection Config | 🏁 completed | 🔗 integrated | v1 | full |
| C-231 | Rich Chat Streaming | ❓ ✅ completed | 🧪 sandbox | v1 | full |
| C-232 | Character Sheet Traits | 🏁 completed | — | v1 | full |
| C-233 | World Generation Wizard | 🏁 completed | — | v1 | full |
| C-234 | Combat Enhancement Dice Initiative | ❓ done | — | v1 | full |
| C-235 | Gm Narrative Director | ❓ done | — | v1 | full |
| C-236 | Agent Pipeline System | 🏁 completed | 🔗 integrated | v1 | full |
| C-237 | Prompt Template Macro System | ❓ completed (2026-07-09) | — | v1 | full |
| C-238 | Lorebook World Info | 🏁 completed | — | v1 | full |
| C-239 | Expression Emotion System | 🏁 completed | — | v1 | full |
| C-240 | Session Management | 🏁 completed | 🔗 integrated | v1 | full |
| C-241 | Chat Modes Address System | 🏁 completed | 🚀 release_verified | v1 | full |
| C-242 | Image Generation Pipeline | 🏁 completed | 🔗 integrated | v1 | full |
| C-243 | Asset Management System | 🏁 completed | 🧪 sandbox | v1 | full |
| C-244 | Connected Chats Cross Mode Bridge | 🏁 completed | 🔗 integrated | v1 | full |
| C-245 | Cyoa Choices Branching Narrative | 🏁 completed | 🔗 integrated | v1 | full |
| C-246 | Export Import System | 🏁 completed | 🔗 integrated | v1 | full |
| C-247 | Custom Agent Creation | 🏁 completed | 🔗 integrated | v1 | full |
| C-248 | Autonomous Npc Behavior Schedules | 🏁 completed | 🔗 integrated | v1 | full |
| C-249 | Music Dj Audio Player | 🏁 completed | 🔗 integrated | v1 | full |
| C-300 | Swarm Director And Workspace Provisioning Lifecycle | ⏳ not_started | — | v1 | full |
| C-301 | Algorithmic Token Router And Ast Footprint Extractor | ⏳ not_started | — | v1 | full |
| C-302 | Coherent Ephemeral Sqlite Wal Scratchpad Engine | ⏳ not_started | — | v1 | full |
| C-303 | Self Healing Visual Test Harness | ⏳ not_started | — | v1 | full |
| C-304 | Ast Aware Behavioral Code Reviewer And Convention Gate | ⏳ not_started | — | v1 | full |
| C-305 | Auto Sandbox Agent Scaffolding Lifecycle | 🏁 completed | — | v1 | full |
| C-306 | Swarm Hardening And Cache Synchronization Refactor | 🏁 completed | — | v1 | full |
| C-307 | Autonomous Scope Explorer And Semantic Dependency Traverser | 🏁 completed | — | v1 | full |
| C-308 | Swarm Post Mortem And Meta Reflective Skill Optimizer | 🏁 completed | — | v1 | full |
| C-309 | Swarm Control Pi Extension And Command Surface Wrapper | 🏁 completed | — | v1 | full |
| C-310 | Automated Backlog Contract Generator | 🏁 completed | — | v1 | full |
| C-311 | Swarm Socket Orchestration | ⏳ not_started | — | v1 | full |
| C-312 | Restore Planning Promotion And Release Truth | 🛠️ implemented | 🔗 integrated | v2 | full |
| C-313 | Introduce The Campaign Aggregate And Boot State Machine | 🛠️ implemented | ❓ `sandbox` | v2 | full |
| C-314 | Establish A Production Game Composition Root And Split God S | 🛠️ implemented | ❓ production | v2 | full |
| C-315 | Define A Versioned Campaign Content Pack And Atomic Loader | 🏁 completed | — | v2 | full |
| C-316 | Build The Authored Emberwatch The Fading Ward Demo Adventure | ✅ verified | — | v2 | full |
| C-317 | Rebuild The Start Menu Around Campaigns Not Personas | 🛠️ implemented | — | v2 | full |
| C-318 | Add One Screen Capability Setup And An Offline Demo Fallback | 🛠️ implemented | — | v2 | full |
| C-319 | Replace Setup With Fast Character Onboarding | 🛠️ implemented | — | v2 | full |
| C-320 | Build The Unified Ai Provider Gateway Offline Byok Service | 🛠️ implemented | — | v2 | full |
| C-321 | Migrate Local Persistence To Turso As The Source Of Truth | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-322 | Wire The Ai Provider Gateway Into Capability Detection And S | 🛠️ implemented | — | v2 | full |
| C-323 | Enforce The Mandatory Text Ai Capability Gate | 🛠️ implemented | — | v2 | full |
| C-324 | Retire Legacy Ai Less Code Paths And Unused Backend Packages | ✅ verified | — | v2 | full |
| C-325 | Ship Real Time Lpc Appearance Preview With Safe Defaults | 🛠️ implemented | — | v2 | full |
| C-326 | Make Game Boot Atomic Observable And Content Driven | 🛠️ implemented | — | v2 | full |
| C-327 | Add In World Onboarding And Unified Interaction Ux | 🛠️ implemented | — | v2 | full |
| C-328 | Integrate Bounded Ai Npc Dialogue With Authored Fallbacks | 🛠️ implemented | ❓ production — the dialogue overlay is already mounted on the production `/game` journey; this contract hardens it in place (no sandbox promotion step required, dev sandbox at `/dev/sandbox/dialogue` is updated alongside) | v2 | full |
| C-329 | Integrate The Demo Quest From Offer Through Reward | 🛠️ implemented | — | v2 | full |
| C-330 | Integrate Deterministic Demo Combat And Declared Skill Check | 🛠️ implemented | — | v2 | full |
| C-331 | Integrate Inventory Equipment Loot And Vendor Into The Demo | 🛠️ implemented | ❓ `integrated` — inventory, vendor, and character-dashboard overlays are already mounted on the production `/game` journey (`game_ui_view.svelte`); this contract hardens them in place. Dev sandboxes `/dev/inventory` and `/dev/vendor` are updated alongside. | v2 | full |
| C-332 | Redesign The Minimal Game Hud And Overlay Navigation | 🛠️ implemented | ❓ `integrated` — the HUD and overlay navigation already live on the production `/game` route; this contract redesigns them in place. Dev sandboxes are updated alongside. | v2 | full |
| C-333 | Simplify Settings With Progressive Disclosure | 🛠️ implemented | — | v1 | full |
| C-334 | Make Local Save Continue Autosave And Recovery Reliable | 🛠️ implemented | — | v2 | full |
| C-335 | Enforce The Playable Demo Release Gate | 🛠️ implemented | — | v2 | full |
| C-336 | Extract A Deterministic Rules Kernel And Typed Game Command | 🛠️ implemented | — | v2 | full |
| C-337 | Complete Character Progression Classes Abilities Skills And | 🛠️ implemented | — | v2 | full |
| C-338 | Deepen Turn Based Combat With Action Economy Statuses And Ta | 🛠️ implemented | — | v2 | full |
| C-339 | Complete Quest Graph Journal Objectives And Reward Pipelines | 🛠️ implemented | — | v2 | full |
| C-340 | Build Party And Companion Gameplay | 🛠️ implemented | — | v2 | full |
| C-341 | Add Relationships Factions Reputation And Persistent Consequ | 🛠️ implemented | — | v2 | full |
| C-342 | Add World Interactables Dungeons Puzzles And Loot Tables | 🛠️ implemented | — | v2 | full |
| C-343 | Promote Rich Chat Ux Into Production Gameplay | 🛠️ implemented | ❓ `integrated` — the production dialogue overlay on `/game` already mounts with streaming AI dialogue (C-328); this contract hardens the rich chat surface in place (no sandbox promotion step) | v2 | full |
| C-344 | Complete Session Recaps Checkpoints And Long Campaign Lifecy | 🛠️ implemented | — | v2 | full |
| C-345 | Add A Campaigncontent Pack Browser And A Second Adventure | 🛠️ implemented | — | v2 | full |
| C-370 | Fix Lpc Paperdoll Base Layering And Neck Alignment | 🛠️ implemented | — | v2 | full |
| C-371 | C 371 | 👍 approved | — | v2 | full |
| C-372 | Fix P0 Lpc Asset Resolution And Unify Resolver | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-373 | Turso Asset Registry Opfs Cache | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-374 | Hub Firestore To Dataconnect | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-375 | Emberwatch Rendering And Assets Overhaul | 🛠️ implemented | ❓ `sandbox` — | v2 | full |
| C-376 | Emberwatch Solidity Rendering Architecture Hardening | 🛠️ implemented | ❓ `integrated` — no dev sandbox required; the production `/game` route + E2E + visual suite are the evidence (C-375 gate re-run) | v2 | full |
| C-377 | Pixel Art Render Correctness | 🛠️ implemented | ❓ `integrated` — the production `/game` route + the existing `emberwatch.visual.ts` suite are the evidence | v2 | full |
| C-378 | Layered Terrain And Autotiling | 🛠️ implemented | ❓ `integrated` — the production `/game` route + `emberwatch.visual.ts` are the evidence | v2 | full |
| C-379 | Collision And Movement Unification | 🛠️ implemented | ❓ `integrated` — `/game` route, `collision_e2e.spec.ts`, `emergent_world.visual.ts` | v2 | full |
| C-380 | Frame Pacing And Click To Move | 🛠️ implemented | ❓ `integrated` — `/game` route + a new Playwright spec | v2 | full |
| C-381 | Content Pipeline Hardening | 🛠️ implemented | ❓ `integrated` — `/game` boot path + hub upload gate | v2 | full |
| C-383 | Data Exposure Hardening | 🏁 completed | — | v2 | full |
| C-384 | Local Sqlite Migration Framework | 🛠️ implemented | — | v2 | full |
| C-385 | Remove Data Connect | 🛠️ implemented | — | v2 | full |
| C-386 | Firestore Removal Local First Client | 🛠️ implemented | — | v2 | full |
| C-387 | Local Postgres Dev Environment | 🛠️ implemented | — | v2 | full |
| C-388 | Image Engine Provider Abstraction | 👍 approved | — | v2 | full |
| C-389 | Runtime Engine Config Offline Tts Tauri | 🛠️ implemented | — | v2 | full |
| C-390 | Local Stack V2 Publishable Topology | 🛠️ implemented | — | v2 | full |
| C-391 | Stack Init Hardware Detection Wizard | 🛠️ implemented | — | v2 | full |
| C-392 | Converge Dev Engine Services With Stack | 👍 approved | — | v2 | full |
| C-393 | Speech To Text Backend Service | 👍 approved | — | v2 | full |
| C-394 | Server Data Plane Neon Drizzle | 🛠️ implemented | — | v2 | full |
| C-395 | R2 Asset Origin Publish Pipeline | 🛠️ implemented | — | v2 | full |
| C-396 | Hub Public Shell Catalog Browse | 🛠️ implemented | — | v2 | full |
| C-400 | Unify Lpc Appearance Resolution | 🛠️ implemented | ❓ `—` | v2 | full |
| C-401 | Stream Dialogue Narrative | 🛠️ implemented | ❓ `—` | v2 | full |
| C-402 | Fix Npc Player Movement Deadlock | 🛠️ implemented | ❓ `—` | v2 | full |
| C-405 | Cut Worldgen From Critical Path | 🛠️ implemented | ❓ `—` | v2 | full |
| C-417 | P1 Polish Batch | 🛠️ implemented | ❓ `—` | v2 | full |
| C-418 | P2 Cleanup And Infrastructure | 👍 approved | ❓ `—` | v2 | full |
| C-419 | P3 Growth Features | 🛠️ implemented | ❓ `—` | v2 | full |
| C-420 | Guided Choice Chips | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-421 | Dice Cards | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-422 | Onboarding Arc | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-423 | Design North Star | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-424 | Unified Message Surfaces | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-425 | Viewmodel Decomposition | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-426 | Cloudflare Native Identity And Hosting | 🛠️ implemented | ❓ `sandbox` | v1 | full |
| C-427 | Local Model Runtime | 🛠️ implemented | ❓ `integrated` | v1 | full |
| C-428 | Lpc Sheet Geometry Unification | 🛠️ implemented | — | v1 | full |
| C-429 | Lpc Sheet Coverage Audit | 🛠️ implemented | — | v1 | full |
| C-430 | Lpc Layer Model And Z Order | 🛠️ implemented | — | v1 | full |
| C-431 | Collect Lpc Behind Pass | 🛠️ implemented | — | v1 | full |
| C-432 | Content Addressed R2 Client Sources | 🛠️ implemented | — | v1 | full |
| C-433 | Catalog Coverage Maps Tilesets Packs | 🛠️ implemented | — | v1 | full |
| C-434 | Registry Backed Maps And Packs | 🛠️ implemented | — | v1 | full |
| C-435 | Debundle Game Data | 🛠️ implemented | — | v1 | full |
| C-436 | Postgres Decommission | 🛠️ implemented | — | v2 | full |
| C-437 | Local Cloudflare Dev Plane | 🛠️ implemented | — | v2 | full |
| C-438 | Restore Pr Checks | 🛠️ implemented | — | v2 | full |
| C-439 | Card Lorebook Import | 🛠️ implemented | — | v2 | full |
| C-440 | Ci Tooling Baseline | 🛠️ implemented | — | v2 | full |
| C-441 | Sops Secrets Migration | 🛠️ implemented | — | v2 | full |
| C-442 | Lpc Core Package Runtime Catalog | 🛠️ implemented | — | v1 | full |
| C-443 | Engine Subpath Entrypoints | 🛠️ implemented | — | v1 | full |
| C-444 | Asset Resolver Seam | 🛠️ implemented | — | v1 | full |
| C-445 | Shared Preview Package | 📝 draft | ❓ `sandbox` | v1 | full |
| C-446 | Hub Catalog Asset Previews | 🛠️ implemented | ❓ `integrated` | v1 | full |
| C-447 | Hub Walkable Map Sandbox | 🛠️ implemented | ❓ `integrated` | v1 | full |
| C-448 | Debundle Content Packs | 🛠️ implemented | — | v1 | full |
| C-449 | Misc Polish Batch | 🛠️ implemented | ❓ `sandbox` | v1 | full |
| C-450 | Contract Pipeline Reconciliation And Drift Guard | 🛠️ implemented | — | v2 | full |
| C-451 | Thin Contract Mode | 🛠️ implemented | — | v1 | full |
| C-452 | Backfill Execution Reports | 🛠️ implemented | ❓ `integrated` — doc-only change, no sandbox route | v2 | full |
| C-453 | Moon Remote Build Cache Cloudflare Workers R2 | 📝 draft | — | v2 | full |
| C-454 | D1 R2 Infrastructure Constants Storage Package | 🛠️ implemented | — | v2 | full |
| C-455 | Apps Backend Cloudflare One Home For Cloudflare Operations | 🛠️ implemented | — | v2 | full |
| C-456 | Group Chat And Systemic Npc Interactions | 🛠️ implemented | — | v1 | full |
| C-457 | Gm Prompt Assembly Upgrade | 🛠️ implemented | — | v1 | full |
| C-458 | In House Memory And Lore Retrieval System | 🛠️ implemented | — | v1 | full |
| C-459 | Ai Gm Narrative Director Enhancements | 🛠️ implemented | — | v2 | full |
| C-460 | Npc Behavioral Autonomy Layer | 🛠️ implemented | — | v2 | full |
| C-461 | Generate Typebox Row Schemas From Drizzle | 🛠️ implemented | ❓ `sandbox` | v2 | thin |
| C-462 | Client Side R2 Save Backup And Restore | 🛠️ implemented | ❓ `sandbox` | v2 | full |
| C-463 | Provider Connection Role Model | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-464 | Account Section And Deletion | 🛠️ implemented | ❓ `integrated` | v2 | full |
| C-465 | Ai Settings Section | 🛠️ implemented | ❓ `integrated` | v1 | full |
| C-466 | Unify Settings Mounts | 🛠️ implemented | — | v1 | full |
| C-467 | Tauri Local Ai Install Wizard | 🛠️ implemented | — | v1 | full |
| C-468 | Agent Test Foundation | 🛠️ implemented | — | v2 | thin |
| C-469 | Revision Bound Validation | 🛠️ implemented | — | v2 | full |
| C-470 | Pipeline Ownership | 🛠️ implemented | — | v2 | full |
| C-471 | Owned Service Lifecycle | 👍 approved | — | v2 | full |
| C-472 | Testable Worker Lifecycle | 👍 approved | — | v2 | full |
| C-473 | Pipeline Usage Ledger | 👍 approved | — | v2 | full |
| C-474 | Role Context Profiles | 🔄 in_progress | — | v2 | thin |
| C-475 | Executable Agent Guidance | 📝 draft | — | v2 | thin |
| C-476 | Strictness Coverage Ratchet | 🛠️ implemented | — | v2 | thin |
| C-477 | Compiled Svelte Lifecycle Tests | 🛠️ implemented | — | v2 | full |
| C-478 | Reproducible Agent Resources | 🛠️ implemented | — | v2 | thin |
| C-479 | Portable Contributor Onboarding | 🛠️ implemented | — | v2 | full |
| C-480 | Agent Evaluation And Routing | 👍 approved | — | v2 | thin |
| C-481 | Ai Configuration Convergence | 🛠️ implemented | — | v2 | full |
| C-482 | Managed Ai Runtime Lifecycle | 🛠️ implemented | — | v2 | full |
| C-483 | Guided Ai Setup | 👍 approved | — | v2 | full |
| C-484 | Capability First Settings | 🛠️ implemented | — | v2 | full |
| C-485 | Production Path Evidence Gate | 🛠️ implemented | — | v2 | full |
| C-486 | Unconditional Release Journey | 🛠️ implemented | — | v2 | full |
| C-487 | Free Text Skill Checks Honour The Character Sheet | 🛠️ implemented | — | v2 | full |
| C-488 | Authored Npc Identity In The Content Pack | 🛠️ implemented | — | v2 | full |
| C-489 | One Authority Path For Consequences | 🛠️ implemented | — | v2 | full |
| C-490 | Transcript Branching Must Not Imply Rewinding The World | 🛠️ implemented | — | v2 | thin |
| C-491 | Committed Narrative Event Record | 🛠️ implemented | — | v2 | full |
| C-492 | Memory Retrieval Correctness And Production Wiring | 👍 approved | — | v2 | full |
| C-493 | Wire Group Scenes Into The Production Party Path | 👍 approved | — | v2 | thin |
| C-494 | One Companion Who Reacts | 🛠️ implemented | — | v2 | full |
| C-495 | Emberwatch Dramatic Structure | 🛠️ implemented | — | v2 | full |
| C-496 | Shared Visual Assets And Playback | 🛠️ implemented | — | v2 | full |
| C-497 | Camera Framing And Default Asset Review | 🛠️ implemented | — | v2 | thin |
| C-498 | Preset Means Character Is Ready | 👍 approved | — | v2 | thin |
| C-499 | Dialogue Intent Envelope Extraction Resilience | 🛠️ implemented | ❓ `integrated` — production route `/game` | v2 | thin |
| C-500 | Combat Overlay Rendering And Engine Stall | 🛠️ implemented | ❓ `integrated` — production route `/game` | v2 | full |
| C-501 | Dialogue Slash Commands | 📝 draft | ❓ `integrated` — production route `/game` | v2 | full |
| C-502 | Minimap Hud | 📝 draft | ❓ `integrated` — production route `/game` | v2 | full |
| C-503 | Quest Marker Hud | 📝 draft | ❓ `integrated` — production route `/game` | v2 | full |
| C-504 | Stable Character Appearance Identity | 🛠️ implemented | — | v2 | full |
| C-505 | Canonical Scene Data And Authoring Boundary | 🛠️ implemented | — | v2 | full |
| C-506 | Emberwatch Visual Readability | 🛠️ implemented | — | v2 | thin |
| C-507 | Map Studio Visual Editor | 🛠️ implemented | ❓ `sandbox` | v2 | full |
| C-508 | Map Studio Drafts And Publishing | 🛠️ implemented | ❓ `sandbox` | v1 | full |
| C-509 | Combat 01 Versioned Schemas And Kernel Authority | ✅ verified | ❓ `—` | v2 | full |
| C-510 | Engine Agnostic Asset Generation And Registry Write Seam | 🛠️ implemented | ❓ `—` | v2 | full |
| C-511 | Local Audio Generation Modality | 🛠️ implemented | ❓ `—` | v2 | full |
| C-512 | Creator Studio And Runtime Asset Generation | 🛠️ implemented | ❓ `—` → `sandbox` once Phase 3 ships the `(dev)/dev/studio` route → `integrated` with the production route + E2E | v2 | full |
| C-513 | End User Asset Publishing And Community Sharing | ❓ implemented — hub intake/moderation/promotion + public browse page, client publish + community browse/import, offline-reload resolution, AC-12 runner, and the C-512 production evidence executed for the first time. Re-verified in the C-513 recovery pass (see the Execution Report). | ❓ `—` | v2 | full |
| C-514 | Combat 02 Real Turn Coordinator And Action Budgets | ✅ verified | ❓ `—` | v2 | full |
| C-515 | Combat 03 World Space Tactical Queries And Previews | ✅ verified | ❓ `—` | v2 | full |
| C-516 | Combat 04 Direct Control Production Vertical Slice | 🛠️ implemented | ❓ `—` | v2 | full |
| C-517 | Generation Request And Format Correctness | 🛠️ implemented | — | v1 | full |
| C-518 | Generation Provenance And Candidate Records | 🛠️ implemented | — | v1 | full |
| C-519 | Durable Asset Jobs And Batch Execution | 🛠️ implemented | — | v1 | full |
| C-520 | Versioned Image Workflows And Asset Preparation | 🛠️ implemented | — | v1 | full |
| C-521 | Music Sfx Generation And Audio Preparation | 🛠️ implemented | — | v1 | full |
| C-522 | Hub And Client Generation Runner Access | 📝 draft | — | v1 | full |
| C-523 | Emberwatch Asset Pilot And Offline Integration | 📝 draft | — | v1 | full |
| C-524 | Optional Hosted Asset Provider Comparison | 📝 draft | — | v1 | full |
| C-525 | Combat 05 Natural Language Intent And Confirmation Ux | ✅ verified | ❓ `—` | v2 | full |
| C-526 | Combat 06 Companion And Enemy Llm Intent Agents | 🛠️ implemented | ❓ `—` | v2 | full |
| C-527 | Coherent Play Shell And Management Navigation | 📝 draft | — | v2 | full |
| C-528 | Player Hud Presets And Layout Editor | 📝 draft | — | v2 | full |
| C-529 | Declarative Theme Runtime And Creator Tools | 📝 draft | — | v2 | full |
| C-530 | Hub Theme Publishing And Installation | 📝 draft | — | v2 | full |

---

### 📦 Archived Contracts

> These 119 legacy v1 contracts were marked completed with no execution report and are not referenced by pending work.
> See `docs/contracts/archived/README.md` for details.

| Contract | Name | Status | Version | Type |
|----------|------|--------|---------|------|
| C-001 | Remove Ai Vendor Dirs | 📦 archived | v1 | full |
| C-002 | Establish Knowledge Dir | 📦 archived | v1 | full |
| C-003 | Establish Pi Setup | 📦 archived | v1 | full |
| C-004 | Migrate Skills | 📦 archived | v1 | full |
| C-005 | Restructure Packages Shared | 📦 archived | v1 | full |
| C-006 | Add Frontend Configs Package | 📦 archived | v1 | full |
| C-007 | Establish Scripts Project | 📦 archived | v1 | full |
| C-008 | Copy Moon Setup | 📦 archived | v1 | full |
| C-009 | Standardize Moon Tsconfig | 📦 archived | v1 | full |
| C-010 | Setup Script | 📦 archived | v1 | full |
| C-012 | Generate Llms And Context | 📦 archived | v1 | full |
| C-013 | Setup Tooling And Mcp | 📦 archived | v1 | full |
| C-016 | Game Engine Boundary | 📦 archived | v1 | full |
| C-017 | Update Knowledge Base | 📦 archived | v1 | full |
| C-018 | Frontend Api Abstraction | 📦 archived | v1 | full |
| C-019 | Macro Slash Engine | 📦 archived | v1 | full |
| C-020 | Spatial Context Injection | 📦 archived | v1 | full |
| C-021 | Engine Package Extraction | 📦 archived | v1 | full |
| C-022 | Worker Ecs Synchronization | 📦 archived | v1 | full |
| C-023 | Dense Spatial Hash | 📦 archived | v1 | full |
| ... | _99 more archived_ | | | |

<details><summary>All 119 archived IDs</summary>

C-024, C-025, C-026, C-027, C-028, C-029, C-030, C-032, C-033, C-034, C-035, C-036, C-037, C-038, C-039, C-040, C-041, C-042, C-043, C-044, C-045, C-046, C-047, C-048, C-049, C-050, C-051, C-052, C-054, C-055, C-057, C-058, C-059, C-060, C-061, C-062, C-063, C-064, C-065, C-066, C-067, C-068, C-069, C-070, C-071, C-072, C-073, C-074, C-076, C-077, C-078, C-079, C-080, C-081, C-101, C-102, C-103, C-104, C-105, C-106, C-107, C-108, C-109, C-110, C-111, C-112, C-113, C-114, C-115, C-116, C-122, C-126, C-130, C-131, C-137, C-139, C-145, C-146, C-147, C-148, C-150, C-151, C-160, C-165, C-166, C-167, C-169, C-170, C-171, C-172, C-174, C-176, C-177, C-178, C-179, MIG-001, MIG-002, MIG-003, MIG-004

</details>

