...
## Overview
Stand up `apps/backend/image` as a standalone ComfyUI microservice using the `yanwk/comfyui-boot` Docker image. Integrate it into the shared `tmux` orchestrator so it can be managed alongside `pwa`, `voice`, and `emulators`. Allocate development ports for the image service and create a test script to verify API health.
