...
- Target System: `apps/frontend/client/src/lib/client/services/media/tts.svelte.ts`

## Detailed Changes

1. **Audio Manager Engine**: Create `apps/frontend/client/src/lib/client/services/media/audio_context_manager.ts`. This file will encapsulate a singleton `AudioContext`. It must include an `unlock()` method bound to a window event listener to satisfy browser autoplay policies.
...
4. **UI Highlighting**: Modify the relevant chat message UI component (e.g., `apps/frontend/client/src/lib/components/chat/message_bubble.svelte` or `tts_player.svelte`) to wrap words in `<span>` tags. Apply a CSS highlight class conditionally based on `tts_service.current_word_index`.
