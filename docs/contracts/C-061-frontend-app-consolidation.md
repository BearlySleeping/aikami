...
- **Code Migration**: Move all game-specific source code from `apps/frontend/game/src/lib/` to `apps/frontend/pwa/src/lib/client/game/`.
...
- **AC1: Code Relocation & Import Fixing**
  ...
  - Then all game logic resides in `apps/frontend/pwa/src/lib/client/game/` and all imports correctly resolve via `$lib/client/game/...` without TypeScript errors.
...
- **AC4: Workspace Deprecation**
  ...
  - Test Hook: Run `moon project pwa` to ensure the workspace graph remains valid and does not reference the deleted project.

## Implementation Notes
1. Start by moving the code from `apps/frontend/game/src/lib` to `apps/frontend/pwa/src/lib/client/game`.
