...
| **Target** | New app `aikami-cache` (Cloudflare Worker + R2), `scripts/src/lib/deploy/deployment_config.ts`, `.moon/workspace.yml`, `.github/actions/setup-environment/action.yml` |
| **Type** | full |
...
| **Dependencies** | Existing Cloudflare Worker deploy pipeline (`cloudflare-worker` serviceType), C-441 (SOPS secrets — token distribution), C-437 (local Cloudflare dev plane, for local Worker testing) |
| **Status** | draft |
| **Promotion** | — |
...
| Moon workspace config | `.moon/workspace.yml` | modify — add top-level `remote:` block |
...
- `deployment_config.ts`: add `aikami-cache` to `APP_CONFIG` with `serviceType: 'cloudflare-worker'`, custom domain `cache.bearlysleeping.com` (staging equivalent following the existing `stg.` subdomain convention used by `client`/`site`/`hub`), and its own dedicated R2 bucket binding (not shared with any existing bucket).
...
- **In Scope:**
  - New `aikami-cache` Worker + dedicated R2 bucket, deployed through the existing `cloudflare-worker` pipeline.
...
|---|---|---|---|---|
| AC-1 | Integration | `apps/backend/worker` or new Worker test dir — `tests/aikami_cache_auth.test.ts` | `cache.bearlysleeping.com/ac/*`, `/cas/*` | Filled during verification |
...
1. **Phase 1 (Worker + R2)**: Build the `aikami-cache` Worker (four routes, bearer-token auth, two-tier tokens), provision the dedicated R2 bucket with the 14-day lifecycle rule, add the `APP_CONFIG` entry, deploy via the existing `cloudflare-worker` pipeline to `cache.bearlysleeping.com`. Verify AC-1 and AC-2.
