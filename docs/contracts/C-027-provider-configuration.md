...
## Detailed Changes
1. **State Management**: Create `apps/frontend/client/src/lib/client/services/settings/ai_settings.svelte.ts`. This singleton must manage `$state` for:
...
2. **Secrets Vault**: Create `apps/frontend/client/src/lib/client/utils/crypto_vault.ts`. Implement an AES-GCM encryption wrapper using the Web Crypto API. API keys must be encrypted at rest in `localStorage` or `IndexedDB`. If the user is authenticated via Firebase, sync the encrypted vault to a secure `user_secrets` subcollection in Firestore.
...
4. **UI Integration**: 
   - Build `apps/frontend/client/src/lib/views/settings/tabs/ai_providers_tab.svelte`.
   - Build `apps/frontend/client/src/lib/views/settings/tabs/instruct_templates_tab.svelte`.
