...
- **Reproduction**:
  ...
  3. Start any encounter: `encounter_system.ts:204–216` only emits `COMBAT_STARTED`; `initCombat` has no production caller except `RETRY_ENCOUNTER` (`ecs_worker.ts:653–654`), and `handleCombatAction` no-ops when turn order is empty (`turn_manager_system.ts:389–391`).
...
- **Baseline tests** (run before starting):
  ...
  - `bun moon run shared-utils:test`, `bun moon run shared-schemas:test`, `bun moon run frontend-engine:test`.
...
- **ECS conventions**: follow the packed SoA component + observers pattern in `packages/frontend/engine/src/components/combat_stats.ts`; reuse the string registry (C-195) for stable combatant IDs.
...
- **Pure rules implementation**: `packages/shared/utils/src/lib/rules/combat_kernel.ts` — stable combat-kernel facade (`createCombatState`, `validateCombatCommand`, `resolveCombatCommand`, `replayCombat`, `findFirstCombatDivergence`) built on the existing RNG and kernel utilities. It must import **only** shared packages; never `@aikami/engine`, client code, ECS, network, or AI.
- **ECS snapshot/apply adapter**: `packages/frontend/engine/src/combat/` — `snapshotCombatState(world, …)` and `applyCombatResult(world, previousState, result)`. The adapter projects and applies; it must not recalculate rules, hit chance, or legality.
- **Stable identity**: add a `CombatIdentity` ECS component carrying a `combatantId: string` set at spawn. The adapter owns a `combatantId ↔ runtime eid` map; raw eids never appear in a `CombatState` or a replay artifact.
...
type ResolveCombatResult = { state: CombatState; events: CombatEvent[] };
...
type ReplayCombatResult = { replay: CombatReplay; finalState: CombatState };

// Exported functions:
//   createCombatState(input): CombatState
//   validateCombatCommand(input: { state; command }): CombatValidationResult
//   resolveCombatCommand(input: { state; command }): ResolveCombatResult
...
`resolveCombatCommand` validates first (returning `CombatValidationResult` semantics), mutates nothing, increments `stateRevision` by exactly 1 on success, advances only the relevant RNG substream, and returns new events. `replayCombat` reconstructs events and final state from `initialState` + `rulesVersion` + `commands` alone.
...
- **Persistence/migration**: N/A — no persistent state is written or read. Schemas are versioned (`schemaVersion`, `rulesVersion`, `replayVersion`) so a future save/replay slice can migrate; legacy saves are untouched.
...
**For this contract:** One outcome — one deterministic authority owns combat mechanics. Schemas, kernel, identity, RNG, and the ECS projection are inseparable parts of that single seam; splitting them would leave the repo mid-extraction with two competing authority paths. 7 ACs, 4 affected projects (`shared/schemas`, `shared/types`, `shared/utils`, `frontend/engine`), no UI. **Size: ok — proceed as one contract.**
...
|---|---|---|---|---|
| AC-1 | Unit | `packages/shared/schemas/src/lib/game/combat/combat_state.test.ts` | `packages/shared/schemas/src/lib/game/combat/` + tooling: `bun moon run shared-schemas:test` | Filled during verification |
...
**Then** movement decrements `movementRemaining` by path length and emits `movementCommitted`; `useAbility` rolls attack on the `actions` substream, clamps damage at 0 HP, emits `attackRolled`/`damageApplied`/`combatantDowned`/`combatantDefeated`; `defend`/`wait` consume the action and emit the correct events; `endTurn` advances the active index (skipping defeated combatants, incrementing `round` on wrap) and emits `turnEnded`/`turnStarted`; `stateRevision` increases by exactly 1 per success; and invalid commands return the correct `CombatInvalidReason` without changing state.

**Evidence Matrix**:
| AC | Test Level | Required Artifact | Production Path | Evidence |
|---|---|---|---|---|
| AC-2 | Unit | `packages/shared/utils/src/lib/rules/__tests__/combat_kernel.test.ts` | `packages/shared/utils/src/lib/rules/combat_kernel.ts#resolveCombatCommand` + tooling: `bun moon run shared-utils:test` | Filled during verification |
...
**Then** lookups resolve to the correct combatant, a recycled eid never maps to a stale combatant ID, and no `CombatState`/replay field contains a raw eid.
...
| AC | Test Level | Required Artifact | Production Path | Evidence |
|---|---|---|---|---|
| AC-4 | Unit | `packages/shared/utils/src/lib/rules/__tests__/combat_kernel.test.ts` | `packages/shared/utils/src/lib/rules/combat_kernel.ts#resolveCombatCommand` + tooling: `bun moon run shared-utils:test` | Filled during verification |
...
| AC | Test Level | Required Artifact | Production Path | Evidence |
|---|---|---|---|---|
| AC-5 | Integration | `packages/shared/utils/src/lib/rules/__tests__/combat_replay.test.ts` | `packages/shared/utils/src/lib/rules/combat_kernel.ts#replayCombat` + tooling: `bun moon run shared-utils:test` | Filled during verification |
...
**Then** `CombatStats` HP and positions are updated solely from the kernel output, the adapter performs no independent hit/damage/legality calculation, applying the same result twice does not double-apply (revision guard), and legacy production combat behavior is unchanged.
...
**Test Hooks**:
- Moon Task: `bun moon run frontend-engine:test`
- Integration: snapshot → resolve attack → apply → assert HP equals kernel `hpAfter`; re-apply rejected.
...
| AC | Test Level | Required Artifact | Production Path | Evidence |
|---|---|---|---|---|
| AC-7 | Unit | `packages/shared/utils/src/lib/rules/__tests__/combat_kernel_purity.test.ts` | `packages/shared/utils/src/lib/rules/combat_kernel.ts` + tooling: `bun moon run shared-utils:test` | Filled during verification |
...
3. **Phase 3 (ECS adapter)**: Add the `CombatIdentity` component and `packages/frontend/engine/src/combat/combat_state_adapter.ts` (`snapshotCombatState`, `applyCombatResult`, `combatantId ↔ eid` map). Add adapter tests and run the existing legacy turn-manager suite unchanged (AC-3, AC-6).
4. **Phase 4 (Validation)**: Run `bun run lint`/`bun run fix`, then `bun moon run shared-schemas:test`, `bun moon run shared-utils:test`, `bun moon run frontend-engine:test`, and `bun moon run :validate`. Update the Execution Report with exact results.
...
- **Q1 — Stable combatant ID source of truth.** Recommendation: use the campaign character ID for player/companions and a deterministic `<encounterId>:<spawnIndex>` for encounter-spawned entities, minted by the adapter. The implementer must confirm this against the content-pack encounter schema (`content_pack.ts`) and C-195 string registry before coding. **Resolved by architecture §8.1/§25; confirm mapping.**
...
Decisions from architecture §25 adopted here: quantized tactical cells over the continuous world (Q1) and named deterministic RNG substreams (Q3). Legacy in-combat save behavior, rules-version retention mechanics, natural-language confirmation default, and companion default are deferred to Combat-04/05/06/08.
