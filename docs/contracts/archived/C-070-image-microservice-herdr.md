...
# Contract: C-070 — Image Microservice & Herdr Orchestration

## Metadata
| Field | Value |
|-------|-------|
| Source | `packages/shared/constants/src/lib/development_ports.ts`, `scripts/src/lib/herdr/` |
| Target | `apps/backend/image/`, `scripts/src/lib/herdr/`, `packages/shared/constants/` |
| Priority | P1 |
...
## Overview
Stand up `apps/backend/image` as a standalone ComfyUI microservice using the `yanwk/comfyui-boot` Docker image. Integrate it into the shared `herdr` orchestrator so it can be managed alongside `client`, `voice`, and `firebase`. Allocate development ports for the image service and create a test script to verify API health.

## Design Reference
- `apps/backend/voice/` (Container setup, package.json `dev:docker` script, test script)
- `scripts/src/lib/herdr/session.ts` (Service definitions and aliases)
...
- **Herdr Orchestrator**: Update the `DevService` union, aliases, and definitions in `scripts/src/lib/herdr/session.ts` to include the `image` service.
...
**AC-2: Herdr Scripts Refactored**
- Given the herdr CLI commands,
- When running `bun herdr:start image`,
- Then a herdr tab named `image` is created, running `bun run dev` in `apps/backend/image`.
...
2.  **Herdr**: Modify `scripts/src/lib/herdr/session.ts` (and related files like `cli.ts` if needed).
...
4.  **Test Script**: Write `apps/backend/image/scripts/check_health.ts`. Fetch `http://localhost:8188/system_stats`. If successful, print "✓ ComfyUI API is responsive". Handle ECONNREFUSED with a friendly message to run `bun herdr:start image`.
...
- ComfyUI takes a moment to boot. The herdr readiness checker (`isPortReady`) expects an HTTP response. `/` might return a 200 (serving the HTML UI), which perfectly satisfies the port checker.
