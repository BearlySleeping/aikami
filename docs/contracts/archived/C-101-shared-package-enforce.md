...
## Goal
Eradicate boundary bleed. The frontend application (`apps/frontend/client`) currently contains domain constants, TypeScript types/interfaces, and Zod schemas that should be shared across the entire monorepo (Firebase functions, AI microservices, etc.). We must migrate these to the `packages/shared/*` workspaces.

## Scope
- **Source:** `apps/frontend/client/src/lib/types`, `apps/frontend/client/src/lib/constants`, and any stray Zod schemas in the frontend.
- **Destinations:**
...
## Steps
1. **Audit:** Identify all constants, types, and schemas currently living inside `apps/frontend/client/src/` that represent domain models (e.g., Character, User, Message, Chat, Settings).
...
5. **Update Imports:** Refactor `apps/frontend/client` to import these assets using the workspace package names:
...
- **Rule 3:** Ensure package.json dependencies are correct. If the frontend needs a shared package, it must be listed in `apps/frontend/client/package.json` as a workspace dependency (e.g., `"@aikami/shared-types": "workspace:*"`).
