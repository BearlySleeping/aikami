...
github:
  ...
  pr_url: "https://github.com/BearlySleeping/aikami/pull/350"
  pr_number: 350
...
Allocated as C-520 during the 2026-09-13 import. The 2026-09-13 review pack proposed it as C-519; the asset-generation series shifted up by one because C-516 is the combat direct-control contract. Baseline: see `docs/reference/asset-generation-review-2026-09.md`.
...
|---|---|---|---|---|
| AC-1 | profile integration + live GPU smoke | Test/log/media report for this scenario | tooling: `bun run --cwd apps/backend/image generate:batch`; client `/studio/assets` after C-512 recovery | Unit + engine tests: `local-ai/src/lib/workflows/workflow_compiler.test.ts` (declaration↔marker binding, node-schema preflight, zero `/prompt` calls on refusal) and `engines/comfyui_engine.test.ts` (profiled generate path). Live GPU smoke NOT executed — `herdr_session start image-comfyui` failed in 120 s and :8189 never answered; named below. |
| AC-2 | unit + Studio smoke | Test/log/media report for this scenario | tooling: `bun run --cwd apps/backend/image generate:batch`; client `/studio/assets` after C-512 recovery | Unit tests: `workflow_profile_registry.test.ts` + `comfyui_engine.test.ts`; production path: `image:test` case "the batch CLI runs a pinned workflow profile and a preparation profile" proving `engineRequests: 0` when `--workflow-profile` targets a non-ComfyUI engine. |
| AC-3 | fixture integration + native-scale contact sheet | Test/log/media report for this scenario | tooling: `bun run --cwd apps/backend/image generate:batch`; client `/studio/assets` after C-512 recovery | `preparation_host.test.ts` (PNG round-trip + determinism) and the `image:test` case driving `bun run --cwd apps/backend/image generate:batch --run --preparation-profile prop-native-alpha`: `media-validation.json` written, staged hash == preparedSha256, rawSha256 != preparedSha256. |
| AC-4 | visual + game scene smoke | Test/log/media report for this scenario | tooling: `bun run --cwd apps/backend/image generate:batch`; client `/studio/assets` after C-512 recovery | Fixture pair in `prepare_image.test.ts` ("a state swap keeps its footprint"): identical canvas and identical `groundContact`, different bytes. In-game preview swap NOT executed — no accepted generated variant exists here; named below. |
| AC-5 | animation fixture + production preview | Test/log/media report for this scenario | tooling: `bun run --cwd apps/backend/image generate:batch`; client `/studio/assets` after C-512 recovery | `sprite_sheet_qa.test.ts` over synthetic LPC sheets read against `@aikami/lpc` constants (missing direction, wrong frame count, empty/duplicate frame, drifting feet, body-size drift, clipping, facing order) plus the always-on native-1x review requirement. Game-side playback NOT executed — no generated sheet exists here. |
| AC-6 | engine integration + walk-behind scene | Test/log/media report for this scenario | tooling: `bun run --cwd apps/backend/image generate:batch`; client `/studio/assets` after C-512 recovery | `atlas_bounds.test.ts` (page budget, extruded-border bounds, cross-page duplicate names, expected-vs-actual counts, terrain capacity, unresolved references). Real pack repack NOT executed — the packer lazy-requires `sharp`, which is not installed, and the packed pages are gitignored build output; named below. |
...
| AC-3 | ✅ (machine) / ⚠️ (live candidate) | Same raw bytes + same profile version produce byte-identical output (`checkPreparationDeterminism`, plus explicit hash equality in the host test), on the real CLI path: `generate:batch --preparation-profile prop-native-alpha` was driven end-to-end and produced `media-validation.json` with matching raw/prepared hashes and the staged file hashing to the prepared hash. Resample methods are pinned per profile; `pixel-art` + a blending method is refused at registry load and reported as `resample-blended`. Ground rectangles, alpha fringes, clipped content, off-centre ground contact and unreadable native-scale artifacts are machine findings with stable codes. Also verified on a real candidate: `generate:batch --run --preparation-profile prop-native-alpha` against the live sd.cpp server on :8188 (the shipped `prop` recipe renders 512x512 at 20 steps) completed in 314 s with `--timeout 600`. The engine returned a fully opaque frame, and preparation honestly REJECTED it with three machine findings (`alpha-required`, `ground-rectangle-detected`, `clipped-content`) instead of promoting it — an auditable rejection reason on real bytes, which is the deliverable. |
...
7. **The pre-push gate was unblocked by syncing three `scripts/` files from `origin/main`.** `moon run :validate --affected --base=origin/main` was red on `scripts:guard-source-file-size` because this branch's base (`0995ad6c2`, the C-519 merge) still carries a 2439-line `contract_pipeline/orchestrator.ts` against a 2335-line reviewed exception ceiling. That file is repo-wide and affected-independent, and C-520 touches no `scripts/` file at all. `origin/main` already fixed it in the reviewed commit `1e966a0cd` ("chore: cleanup contract pipeline"), which extracted `feedback.ts` and `run_setup.ts` and brought `orchestrator.ts` to 2213 lines. Those three files were checked out from `origin/main` verbatim, so the branch now matches main for a directory it has no business diverging from; no C-520 file is involved and the change is already reviewed on main. With that, `:fix` reports 0 diagnostics and `:validate --affected --base=origin/main` exits 0 (47 tasks).
...
- Guards: `bun run guard` — type-safety, MVVM, service-conventions, data-plane, test-boundary, view-model-composition, image-component and source-file-size all pass (2901 files checked, 38 baselined, 125 non-failing warnings). The previously red `guard-source-file-size` entry for `scripts/src/lib/agents/contract_pipeline/orchestrator.ts` was resolved by checking out main's already-reviewed extraction of that file (see Deviation 7); every file created or modified by this contract is within budget.
- Pre-push gate: `bun moon run :fix --affected --base=origin/main` → 40 tasks, 0 diagnostics. `bun moon run :validate --affected --base=origin/main` → exit 0, 47 tasks completed.
- `scripts:test` — 1196 pass, 2 skip, 2 fail, both pre-existing and independent of this contract: (a) `pack index reconciliation > asset_hashes.json hashes are fresh for the manifest and every map` compares the recorded hash `0c1f6c8d…` against the manifest's current `09b6e149…` and fails identically with and without the pipeline environment, on data (`content/packs/emberwatch`, `asset_hashes.json`) this branch does not touch; (b) `pre_commit checkPlaintextSecrets > fails closed…` fails only because `CONTRACT_PIPELINE_WORKSPACE_PATH`/`CONTRACT_PIPELINE_ROLE` are exported by the pipeline itself, so the pre-commit hook's workspace-boundary guard fires before the secret check the test asserts — it passes with those two variables unset.
...
- Baseline regression: 0 new failures. Pre-existing environment failures are unchanged and unrelated to this contract:
  1. `frontend-engine:test` — 1405 pass, 3 fail, all three in `emberwatch_content_audit.test.ts` asserting that `atlas.json`, `props.webp` and `props.json` exist under `apps/frontend/client/static/game-data/sprites/tilesets/`. That directory is gitignored (`.gitignore:259` ignores the whole `static/game-data/` tree) and absent from any fresh worktree, so the assertions cannot pass before the atlas build runs. This contract changes neither the audit test nor the packer.

### Blocked evidence (named, not claimed)

| Gate | Exact status |
|---|---|
| Live ComfyUI + FLUX.2-klein-4B (AC-1) | Re-confirmed dead during verification (`curl http://127.0.0.1:8189/object_info` → HTTP 000 on 5 attempts; the `image-comfyui` herdr entry aliases the sd.cpp service on :8188). `herdr_session start image-comfyui` → "Services failed to start within 120s: image-comfyui"; the service log shows the container created and attaching but no healthy endpoint. `docker ps` lists only `aikami-local-stack-image-1` (`127.0.0.1:8188`, sd-server). `http://127.0.0.1:8189/object_info` never answered (11 attempts over 55 s). No ComfyUI `/object_info` is therefore available to validate the 4B graph against. |
| Real 512×512 candidate through preparation (AC-3) | **Resolved during verification.** At the 300 s default deadline the CPU-only sd.cpp render was cut off and the job honestly reported `reconciliation_required`/`submission_unconfirmed` rather than silently retrying. Re-run with `--timeout 600` it completed in 314 s on the documented command (see the AC-3 row). |
