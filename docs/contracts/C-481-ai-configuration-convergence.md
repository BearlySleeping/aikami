...
Offline/security: no sign-in/cloud boot dependency. Keys remain provider-owned and vault-encrypted at rest, not copied into model records, diagnostics or discovery caches. Validate endpoint schemes and preserve meaningful paths/ports; reject URL user-info credentials and never forward auth to an unapproved changed endpoint/redirect origin. Keyless local/LAN endpoints work, while optional auth is supported without deriving requirements from locality. Do not weaken browser CORS/mixed-content/CSP or native permission policies to make a test pass.
