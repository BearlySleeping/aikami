...
2. **PWA Auth Handoff (Device Flow)**:
   ...
   - **PWA**: User logs in normally. PWA writes a secure Custom Token or Session Token to the `XYZ` document.
...
## Acceptance Criteria
- **Given** the user launches the game unauthenticated, **When** they request login, **Then** a shortcode is displayed directing them to the PWA.
- **Given** the user logs into the PWA with the shortcode, **Then** the game automatically receives the auth payload and authenticates its internal REST client without a refresh.
...
- The PWA auth endpoint must enforce strict CORS and origin validation when generating or passing custom tokens back to the game.
