...
- Client Tauri Integration: `apps/frontend/client/src-tauri/`
...
2. **Deprecation**: Delete the redundant `apps/frontend/client/src/lib/game/` directory entirely.
3. **Workspace Linking**: 
   - Update `packages/engine/package.json` with name `@aikami/engine`.
   - Add `@aikami/engine: workspace:*` to `apps/frontend/client/package.json` and `apps/frontend/game/package.json`.
...
5. **Client Mount Component**: Create `apps/frontend/client/src/lib/components/game/game_canvas.svelte`. This component will import `initGame()` from `@aikami/engine` and attach the PixiJS application to a bound `<canvas>` ref on `onMount`.

## Acceptance Criteria
- **Given** the monorepo has been restructured.
- **When** running `moon run client:dev`.
...
## Watch Points
- **Asset Loading**: PixiJS asset paths must be absolute or correctly resolved by both Vite (game) and SvelteKit (Client). You may need to inject a `basePath` config when initializing the engine.
