...
- PWA Tauri Integration: `apps/frontend/client/src-tauri/`
...
5. **PWA Mount Component**: Create `apps/frontend/client/src/lib/components/game/game_canvas.svelte`. This component will import `initGame()` from `@aikami/engine` and attach the PixiJS application to a bound `<canvas>` ref on `onMount`.
