...
This document provides a high-level overview of the technical architecture of the Aikami project. It was first written for the July 2026 architecture review and refreshed during the 2026-09 documentation pass. Verify file paths and versions against the checkout before relying on them — the tree moves.
...
│  configs │ engine │ ai-gateway │ storage │ preview │ theme │ services │ utils  │
│  components                                                       │
└──────────────────────────────────────────────────────────────────┘
```
...
- **SQLite compatibility**: Standard SQL in a local store, with embedded-replica sync as an optional path when cloud sync is enabled.
- **Database abstraction**: All client database access goes through the storage adapters in `packages/frontend/storage` (`TursoStorageAdapter`, `LocalDatabaseFactory`) — never direct SDK calls.
...
- AI client implementations live in `apps/frontend/client/src/lib/services/ai/` — engine stays pure ECS/Canvas (C-214)
...
- Access through storage adapters in `packages/frontend/storage` (e.g. `TursoStorageAdapter`)
...
- Schema in `packages/backend/database/src/lib/schema.ts` (Drizzle, sqlite dialect)
...
| `frontend/storage` | frontend | Turso/libSQL local database adapters, asset registry, OPFS cache |
...
| `frontend/local-runtime` | frontend | Local model runtime discovery, download, and lifecycle (C-482) |
| `frontend/preview` | frontend | Shared LPC/map/asset preview surfaces for dev, hub, and studio (C-445) |
| `frontend/theme` | frontend | Declarative theme runtime, compiler, and packaged theme archives (C-529) |
| `frontend/utils` | frontend | Browser utilities |
| `frontend/components` | frontend | Shared Svelte 5 UI components |

### 4. AI Integration

- **One gateway, three modes (C-320)**: All text, image, and voice generation goes through `AiProviderGateway` (`packages/frontend/ai-gateway`) with `offline` (local `llama.cpp`/`sd-server`/`sherpa-onnx` engines), `byok` (user-supplied cloud key), and `service` (Aikami-hosted) modes. Product code depends on the interface, never on the active mode.
...
- **Local models**: `llama.cpp` (text), `sd-server` (image), and `sherpa-onnx`/Kokoro (voice) run as Docker microservices via herdr (C-390); Ollama and ComfyUI are opt-in swaps. Image and voice stay optional; LPC sprites cover the visual baseline with zero AI dependency.
...
| **Moon** | Task orchestration, caching, dependency management |
| **Biome** | Linting and formatting |
| **TypeScript 6.0** | Type checking across all projects |
| **TypeBox** | Runtime validation (shared schemas, types, mocks) |
| **Playwright** | Browser E2E testing |
| **`bun test`** | Unit testing for libraries and apps |
...
bun moon run :validate   # lint + format + typecheck (the CI gate)
bun run test             # all project test suites
